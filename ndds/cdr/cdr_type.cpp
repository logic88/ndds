...
bool
compatibleTransports(const NDDS::DCPS::TransportLocatorSeq& s1,
                     const NDDS::DCPS::TransportLocatorSeq& s2)
{
...
bool
compatibleQOS(NDDS::DCPS::IncompatibleQosStatus* writerStatus,
              ...
              DDS::SubscriberQos const * const subQos)
{
  ...
  if (!compatibleTransports(pubTLS, subTLS)) {
    ...
    increment_incompatibility_count(writerStatus,
                                    NDDS::TRANSPORTTYPE_QOS_POLICY_ID);
...
compatibleQOS(const DDS::PublisherQos*  pubQos,
              ...
              NDDS::DCPS::IncompatibleQosStatus* writerStatus,
              NDDS::DCPS::IncompatibleQosStatus* readerStatus)
{
  bool compatible = true;
...
bool repr_to_encoding_kind(DDS::DataRepresentationId_t repr, Encoding::Kind& kind)
{
  ...
  case DDS::XCDR2_DATA_REPRESENTATION:
    ...
    break;
  case NDDS::DCPS::UNALIGNED_CDR_DATA_REPRESENTATION:
  ...
  bool cdr_encapsulated)
{
  if (qos.length() == 0) {
    qos.length(1);
    qos[0] = cdr_encapsulated ? DDS::XCDR2_DATA_REPRESENTATION : NDDS::DCPS::UNALIGNED_CDR_DATA_REPRESENTATION;
  }
}
...
void set_reader_effective_data_rep_qos(DDS::DataRepresentationIdSeq& qos)
{
  if (qos.length() == 0) {
    ...
    qos[2] = NDDS::DCPS::UNALIGNED_CDR_DATA_REPRESENTATION;
  }
}
