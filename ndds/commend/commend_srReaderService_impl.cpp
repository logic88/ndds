...
MinimalMemberDetail::MinimalMemberDetail(const NDDS_STRING& name)
{
...
{
#define NDDS_BRANCH_ACTIVATE(T, N) \
  active_ = new(N ## _) T;            \
  if (other) N() = other->N();        \
  break
  ...
  case TI_STRING16_SMALL:
    NDDS_BRANCH_ACTIVATE(StringSTypeDefn, string_sdefn);
  case TI_STRING8_LARGE:
  case TI_STRING16_LARGE:
    NDDS_BRANCH_ACTIVATE(StringLTypeDefn, string_ldefn);
  case TI_PLAIN_SEQUENCE_SMALL:
    NDDS_BRANCH_ACTIVATE(PlainSequenceSElemDefn, seq_sdefn);
  case TI_PLAIN_SEQUENCE_LARGE:
    NDDS_BRANCH_ACTIVATE(PlainSequenceLElemDefn, seq_ldefn);
  case TI_PLAIN_ARRAY_SMALL:
    NDDS_BRANCH_ACTIVATE(PlainArraySElemDefn, array_sdefn);
  case TI_PLAIN_ARRAY_LARGE:
    NDDS_BRANCH_ACTIVATE(PlainArrayLElemDefn, array_ldefn);
  case TI_PLAIN_MAP_SMALL:
    NDDS_BRANCH_ACTIVATE(PlainMapSTypeDefn, map_sdefn);
  case TI_PLAIN_MAP_LARGE:
    NDDS_BRANCH_ACTIVATE(PlainMapLTypeDefn, map_ldefn);
  case TI_STRONGLY_CONNECTED_COMPONENT:
    NDDS_BRANCH_ACTIVATE(StronglyConnectedComponentId, sc_component_id);
  case EK_COMPLETE:
  case EK_MINIMAL:
  ...
  default:
    NDDS_BRANCH_ACTIVATE(ExtendedTypeDefn, extended_defn);
  }
}
...
#define NDDS_BRANCH_RESET(T) static_cast<T*>(active_)->~T(); break
  switch (kind_) {
  case TI_STRING8_SMALL:
  case TI_STRING16_SMALL:
    NDDS_BRANCH_RESET(StringSTypeDefn);
  case TI_STRING8_LARGE:
  case TI_STRING16_LARGE:
    NDDS_BRANCH_RESET(StringLTypeDefn);
  case TI_PLAIN_SEQUENCE_SMALL:
    NDDS_BRANCH_RESET(PlainSequenceSElemDefn);
  case TI_PLAIN_SEQUENCE_LARGE:
    NDDS_BRANCH_RESET(PlainSequenceLElemDefn);
  case TI_PLAIN_ARRAY_SMALL:
    NDDS_BRANCH_RESET(PlainArraySElemDefn);
  case TI_PLAIN_ARRAY_LARGE:
    NDDS_BRANCH_RESET(PlainArrayLElemDefn);
  case TI_PLAIN_MAP_SMALL:
    NDDS_BRANCH_RESET(PlainMapSTypeDefn);
  case TI_PLAIN_MAP_LARGE:
    NDDS_BRANCH_RESET(PlainMapLTypeDefn);
  case TI_STRONGLY_CONNECTED_COMPONENT:
    NDDS_BRANCH_RESET(StronglyConnectedComponentId);
  case EK_COMPLETE:
  case EK_MINIMAL:
  ...
  case TK_CHAR16:
    break; // no-op, no member selected
  default:
    NDDS_BRANCH_RESET(ExtendedTypeDefn);
  }
}
...
{
  NDDS_ASSERT(type_object.kind == EK_MINIMAL || type_object.kind == EK_COMPLETE);
    ...
    default:
      if (must_understand) {
        if (DCPS_debug_level >= 8) {
          ACE_DEBUG((LM_DEBUG, ACE_TEXT("(%P|%t) unknown must_understand field(%u) in NDDS::XTypes::TypeInformation\n"), member_id));
        }
        return false;
      }
