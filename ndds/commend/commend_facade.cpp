...
bool DynamicType::test_equality_i(const DynamicType& rhs, DynamicTypePtrPairSeen& dt_ptr_pair) const
{
  ...
  if (have_seen == dt_ptr_pair.end()) {
    ...
    return
      NDDS::XTypes::test_equality_i(descriptor_, rhs.descriptor_, dt_ptr_pair) &&
      NDDS::XTypes::test_equality_i(member_by_name, rhs.member_by_name, dt_ptr_pair) &&
      NDDS::XTypes::test_equality_i(member_by_id, rhs.member_by_id, dt_ptr_pair);
  }
  return true;
}
...
DynamicType_rch DynamicType::get_base_type()
{
  if (get_kind() != TK_ALIAS) {
    return DynamicType_rch(this, NDDS::DCPS::inc_count());
  }
