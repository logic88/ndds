...
#ifndef NDDS_DCPS_XTYPES_TYPE_OBJECT_H
#define NDDS_DCPS_XTYPES_TYPE_OBJECT_H
...
namespace XTypes {
  ...
  typedef NDDS_STRING MemberName;
  ...
  typedef NDDS_STRING QualifiedTypeName;
  ...
  NDDS_Dcps_Export DCPS::String equivalence_hash_to_string(const EquivalenceHash& hash);
  ...
  class NDDS_Dcps_Export TypeIdentifier : public DCPS::PoolAllocationBase {
  public:
...
#define NDDS_UNION_ACCESSORS(T, N)                         \
    const T& N() const { return *static_cast<T*>(active_); }  \
    T& N() { return *static_cast<T*>(active_); }
    NDDS_UNION_ACCESSORS(StringSTypeDefn, string_sdefn);
    NDDS_UNION_ACCESSORS(StringLTypeDefn, string_ldefn);
    NDDS_UNION_ACCESSORS(PlainSequenceSElemDefn, seq_sdefn);
    NDDS_UNION_ACCESSORS(PlainSequenceLElemDefn, seq_ldefn);
    NDDS_UNION_ACCESSORS(PlainArraySElemDefn, array_sdefn);
    NDDS_UNION_ACCESSORS(PlainArrayLElemDefn, array_ldefn);
    NDDS_UNION_ACCESSORS(PlainMapSTypeDefn, map_sdefn);
    NDDS_UNION_ACCESSORS(PlainMapLTypeDefn, map_ldefn);
    NDDS_UNION_ACCESSORS(StronglyConnectedComponentId, sc_component_id);
    NDDS_UNION_ACCESSORS(EquivalenceHash, equivalence_hash);
    NDDS_UNION_ACCESSORS(ExtendedTypeDefn, extended_defn);
#undef NDDS_UNION_ACCESSORS
...
#define NDDS_UNION_MEMBER(T, N) char N ## _[sizeof(T)]
      NDDS_UNION_MEMBER(StringSTypeDefn, string_sdefn);
      NDDS_UNION_MEMBER(StringLTypeDefn, string_ldefn);
      NDDS_UNION_MEMBER(PlainSequenceSElemDefn, seq_sdefn);
      NDDS_UNION_MEMBER(PlainSequenceLElemDefn, seq_ldefn);
      NDDS_UNION_MEMBER(PlainArraySElemDefn, array_sdefn);
      NDDS_UNION_MEMBER(PlainArrayLElemDefn, array_ldefn);
      NDDS_UNION_MEMBER(PlainMapSTypeDefn, map_sdefn);
      NDDS_UNION_MEMBER(PlainMapLTypeDefn, map_ldefn);
      NDDS_UNION_MEMBER(StronglyConnectedComponentId, sc_component_id);
      NDDS_UNION_MEMBER(EquivalenceHash, equivalence_hash);
      NDDS_UNION_MEMBER(ExtendedTypeDefn, extended_defn);
#undef NDDS_UNION_MEMBER
    };
  ...
  struct AnnotationParameterValue {
    ...
    NDDS_STRING string8_value;
    NDDS_WSTRING string16_value;
    ExtendedAnnotationParameterValue extended_value;
    ...
    explicit AnnotationParameterValue(const NDDS_STRING& value)
      : kind(TK_STRING8)
      , string8_value(value)
    {}

    explicit AnnotationParameterValue(const NDDS_WSTRING& value)
      : kind(TK_STRING16)
  ...
  struct AppliedVerbatimAnnotation {
    NDDS_STRING placement;
    NDDS_STRING language;
    NDDS_STRING text;

    AppliedVerbatimAnnotation() {}

    AppliedVerbatimAnnotation(const NDDS_STRING& a_placement,
                              const NDDS_STRING& a_language,
                              const NDDS_STRING& a_text)
  ...
  // --- Aggregate types: ------------------------------------------------
  struct NDDS_Dcps_Export AppliedBuiltinMemberAnnotations {
  ...
  struct NDDS_Dcps_Export MinimalMemberDetail {
    NameHash name_hash;
    ...
    explicit MinimalMemberDetail(const NDDS_STRING& name);
  };
  ...
  NDDS_Dcps_Export
  ACE_CDR::ULong hash_member_name_to_id(const NDDS_STRING& name);

  NDDS_Dcps_Export
  void hash_member_name(NameHash& name_hash, const NDDS_STRING& name);
  ...
  typedef NDDS_MAP(TypeIdentifier, TypeObject) TypeMap;
  ...
  void compute_dependencies(const TypeMap& type_map,
                            ...
                            NDDS_SET(TypeIdentifier)& dependencies);
...
#endif  /* NDDS_DCPS_XTYPES_TYPE_OBJECT_H */
