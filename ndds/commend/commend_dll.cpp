  ...
  if (dt->get_descriptor().extensibility_kind == MUTABLE &&
    ...
    if (DCPS::log_level >= DCPS::LogLevel::Notice) {
      ACE_ERROR((LM_NOTICE, "(%P|%t) NOTICE: DynamicData::check_xcdr1_mutable: "
                 "XCDR1 mutable is not currently supported in NDDS\n"));
    }
    return false;
  }
...
bool print_dynamic_data(DynamicData& dd, DCPS::String& type_string, DCPS::String& indent)
{
  ...
  case TK_BITMASK:
    if (DCPS::log_level >= DCPS::LogLevel::Notice) {
      ACE_ERROR((LM_NOTICE, "(%P|%t) NOTICE: print_dynamic_data: Bitmask is an unsupported type in NDDS\n"));
    }
    break;
  case TK_BITSET:
    if (DCPS::log_level >= DCPS::LogLevel::Notice) {
      ACE_ERROR((LM_NOTICE, "(%P|%t) NOTICE: print_dynamic_data: Bitset is an unsupported type in NDDS\n"));
    }
    break;
  case TK_SEQUENCE: {
