...
namespace {
  using NDDS::DCPS::Encoding;
  const Encoding encoding_unaligned_native(Encoding::KIND_UNALIGNED_CDR);
}

NDDS_BEGIN_VERSIONED_NAMESPACE_DECL

NDDS::DCPS::TcpDataLink::TcpDataLink(
  const ACE_INET_Addr& remote_address,
  NDDS::DCPS::TcpTransport&  transport_impl,
  Priority priority,
  bool        is_loopback,
  bool        is_active)
...
void
NDDS::DCPS::TcpDataLink::send_i(TransportQueueElement* element, bool relink)
{
...
void
NDDS::DCPS::TcpDataLink::send_stop_i(RepoId repoId)
{
  ACE_Guard<ACE_Thread_Mutex> guard(stopped_clients_mutex_);
...
bool
NDDS::DCPS::TcpDataLink::check_active_client(const RepoId& local_id)
{
  ACE_Guard<ACE_Thread_Mutex> guard(stopped_clients_mutex_);
...
void
NDDS::DCPS::TcpDataLink::client_stop(const RepoId& local_id)
{
  ACE_Guard<ACE_Thread_Mutex> guard(stopped_clients_mutex_);
...
void
NDDS::DCPS::TcpDataLink::pre_stop_i()
{
...
//this new connection.
int
NDDS::DCPS::TcpDataLink::reuse_existing_connection(const TcpConnection_rch& connection)
{
...
int
NDDS::DCPS::TcpDataLink::reconnect(const TcpConnection_rch& connection)
{
...
void
NDDS::DCPS::TcpDataLink::send_graceful_disconnect_message()
{
...
void
NDDS::DCPS::TcpDataLink::set_release_pending(bool flag)
{
  this->release_is_pending_ = flag;
}

bool
NDDS::DCPS::TcpDataLink::is_release_pending() const
{
#ifdef ACE_HAS_CPP11
...
bool
NDDS::DCPS::TcpDataLink::handle_send_request_ack(TransportQueueElement* element)
{
  if (Transport_debug_level >= 1) {
    ...
    ACE_DEBUG((LM_DEBUG, ACE_TEXT("(%P|%t) TcpDataLink::handle_send_request_ack(%@) sequence number %q, publication_id=%C\n"),
      element, element->sequence().getValue(), NDDS_STRING(converter).c_str()));
  }
...
void
NDDS::DCPS::TcpDataLink::ack_received(const ReceivedDataSample& sample)
{
  ...
  if (Transport_debug_level >= 1) {
    ...
    ACE_DEBUG((LM_DEBUG, ACE_TEXT("(%P|%t) TcpDataLink::ack_received() received sequence number %q, publiction_id=%C\n"),
      sequence.getValue(), NDDS_STRING(converter).c_str()));
  }
...
void
NDDS::DCPS::TcpDataLink::request_ack_received(const ReceivedDataSample& sample)
{
...
void
NDDS::DCPS::TcpDataLink::do_association_actions()
{
...
void
NDDS::DCPS::TcpDataLink::send_association_msg(const RepoId& local, const RepoId& remote)
{
  DataSampleHeader header_data;
...
void
NDDS::DCPS::TcpDataLink::drop_pending_request_acks()
{
  ACE_Guard<ACE_SYNCH_MUTEX> guard(pending_request_acks_lock_);
  PendingRequestAcks::iterator it;
  for (it = pending_request_acks_.begin(); it != pending_request_acks_.end(); ++it){
...
NDDS::DCPS::TcpDataLink::send_strategy()
{
  GuardType guard(strategy_lock_);
  return static_rchandle_cast<NDDS::DCPS::TcpSendStrategy>(send_strategy_);
}

NDDS::DCPS::TcpReceiveStrategy_rch
NDDS::DCPS::TcpDataLink::receive_strategy()
{
  GuardType guard(strategy_lock_);
  return static_rchandle_cast<NDDS::DCPS::TcpReceiveStrategy>(receive_strategy_);
}
int
NDDS::DCPS::TcpDataLink::make_reservation(const RepoId& remote_subscription_id,
...
int
NDDS::DCPS::TcpDataLink::make_reservation(const RepoId& remote_publication_id,
