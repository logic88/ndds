...
{
  using namespace NDDS::FileSystemStorage;
...
Directory::Ptr Directory::get_dir(const NDDS_VECTOR(NDDS_STRING)& path)
{
  Directory::Ptr dir = rchandle_from(this);
  typedef NDDS_VECTOR(NDDS_STRING)::const_iterator iterator;
...
ACE_TString Directory::add_entry()
{
  ...
  typedef NDDS_MAP(unsigned int, unsigned int)::iterator iterator;
  ...
  for (iterator iter = overflow_.begin(), end = overflow_.end();
       iter != end; ++iter) {
    if (iter->second < NDDS_FILESYSTEMSTORAGE_MAX_FILES_PER_DIR) {
      ++iter->second;
...
NDDS_STRING Directory::name() const
{
