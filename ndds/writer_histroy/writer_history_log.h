...
#ifndef NDDS_DCPS_FILTER_EVALUATOR_H
#define NDDS_DCPS_FILTER_EVALUATOR_H
...
class NDDS_Dcps_Export FilterEvaluator : public RcObject {
public:
  ...
  NDDS_VECTOR(NDDS_STRING) getOrderBys() const;
  ...
  struct NDDS_Dcps_Export DataForEval {
...
private:
  ...
  struct NDDS_Dcps_Export DeserializedForEval : DataForEval {
  ...
  struct SerializedForEval : DataForEval {
    ...
    bool swap_, cdr_;
    mutable NDDS_MAP(NDDS_STRING, Value) cache_;
  ...
  NDDS_VECTOR(NDDS_STRING) order_bys_;
...
class NDDS_Dcps_Export MetaStruct {
public:
  virtual ~MetaStruct();
