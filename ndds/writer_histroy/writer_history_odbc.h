...
#ifndef NDDS_DCPS_GUIDCONVERTER_H
#define NDDS_DCPS_GUIDCONVERTER_H
 ...
 * other types used within NDDS.  This class handles opaque GUID_t values
 ...
 * ---- ---------------------    --------------------------
 *   0  GUID_t.guidPrefix[ 0] == VendorId_t == 0x01 for OCI (used for NDDS)
 *   1  GUID_t.guidPrefix[ 1] == VendorId_t == 0x03 for OCI (used for NDDS)
 ...
 */
class NDDS_Dcps_Export GuidConverter {
public:
  ...
  operator NDDS_STRING() const;
#ifdef DDS_HAS_WCHAR
  operator std::wstring() const;
#endif

  NDDS_STRING uniqueParticipantId() const;

protected:
  const GUID_t guid_;
};

struct NDDS_Dcps_Export LogGuid {
...
NDDS_Dcps_Export std::ostream&
operator<<(std::ostream& os, const NDDS::DCPS::GuidConverter& rhs);

#ifdef DDS_HAS_WCHAR
NDDS_Dcps_Export std::wostream&
operator<<(std::wostream& os, const NDDS::DCPS::GuidConverter& rhs);
#endif //DDS_HAS_WCHAR
#endif //NDDS_SAFETY_PROFILE
