...
GuidConverter::entityKind() const
{
  ...
  case ENTITYKIND_NDDS_NIL_WRITER:
  ...
  case ENTITYKIND_NDDS_PUBLISHER:
    return KIND_PUBLISHER;

  case ENTITYKIND_NDDS_SUBSCRIBER:
    return KIND_SUBSCRIBER;

  case ENTITYKIND_NDDS_USER:
    return KIND_USER;
...
GuidConverter::operator NDDS_STRING() const
{
  NDDS_STRING ret(to_string(guid_));
  ret += "(";
...
GuidConverter::operator std::wstring() const
{
  ...
  const NDDS_STRING guid_str(to_string(guid_));
