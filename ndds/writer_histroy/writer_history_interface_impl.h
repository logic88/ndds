...
#ifndef NDDS_DCPS_FILESYSTEMSTORAGE_H
#define NDDS_DCPS_FILESYSTEMSTORAGE_H
...
#ifndef NDDS_FILESYSTEMSTORAGE_MAX_FILES_PER_DIR
#define NDDS_FILESYSTEMSTORAGE_MAX_FILES_PER_DIR 512
#endif
...
using NDDS::DCPS::RcObject;
using NDDS::DCPS::RcHandle;

class File;

class NDDS_Dcps_Export Directory : public RcObject {
public:
...
private:
  ...
  typedef NDDS_MAP(ACE_TString, ACE_TString) Map;
  ...
  private:
    ...
    typename Item::Ptr deref() const {
      if (item_.is_nil()) {
        item_ = NDDS::DCPS::make_rch<Item>(outer_->full_path(delegate_->second),
...
public:
  ...
  Directory::Ptr get_dir(const NDDS_VECTOR(NDDS_STRING)& path);
...
private:
  ...
  friend RcHandle<T> NDDS::DCPS::make_rch(const U0&, const U1&, const U2&);
  ...
  NDDS_MAP(unsigned int, unsigned int) overflow_;

  Map files_, dirs_; // logical -> physical
  NDDS_MAP(ACE_TString, unsigned int) long_names_;
  // phys. prefix (before '.') -> next available counter #
};

class NDDS_Dcps_Export File : public RcObject {
public:
...
private:
  friend class Directory;
  template <typename T, typename U0, typename U1, typename U2>
  friend NDDS::DCPS::RcHandle<T> NDDS::DCPS::make_rch(const U0&, const U1&, const U2&);
