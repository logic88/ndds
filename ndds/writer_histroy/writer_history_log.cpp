...
using namespace NDDS::DCPS::FilterExpressionGrammar;
...
public:
  ...
  {
    for (NDDS_VECTOR(EvalNode*)::const_iterator i = children_.begin(); i != children_.end(); ++i) {
      EvalNode* child = *i;
...
protected:
  NDDS_VECTOR(EvalNode*) children_;
};
...
{
  const NDDS_MAP(NDDS_STRING, Value)::const_iterator iter = cache_.find(field);
  if (iter != cache_.end()) {
    return iter->second;
  }
  ...
  cache_.insert(std::make_pair(NDDS_STRING(field), v));
  return v;
}
...
{
  for (NDDS_VECTOR(NDDS_STRING)::const_iterator i = order_bys_.begin(); i != order_bys_.end(); ++i) {
...
namespace {
  ...
  public:
    explicit LiteralInt(AstNode* fnNode)
      : value_(0, true)
    {
      const NDDS_STRING strVal = toString(fnNode);
  ...
  public:
    ...
    explicit Call(const NDDS_STRING& name)
    {
      if (name == MOD) {
        op_ = OP_MOD;
      } else {
  ...
  public:
    ...
    {
      Value left = children_[0]->eval(data);
      NDDS_ASSERT(left.type_ == Value::VAL_BOOL);
      switch (op_) {
      case LG_NOT:
        return !left.b_;
      case LG_AND:
...
FilterEvaluator::walkAst(const FilterEvaluator::AstNodeWrapper& node)
{
  if (node->TypeMatches<CompPredDef>()) {
    ...
    if (a == 1) {
      ...
      NDDS_ASSERT(child(node, 0)->TypeMatches<NOT>());
