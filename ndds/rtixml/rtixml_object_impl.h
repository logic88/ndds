...
#ifndef NDDS_DCPS_TRANSPORT_RTPS_UDP_RTPSUDPDATALINK_H
#define NDDS_DCPS_TRANSPORT_RTPS_UDP_RTPSUDPDATALINK_H
...
typedef NDDS_MAP_CMP(RepoId, SeqReaders, GUID_tKeyLessThan) WriterToSeqReadersMap;

class NDDS_Rtps_Udp_Export RtpsUdpDataLink
...
private:
...
#ifdef ACE_HAS_CPP11
  typedef NDDS_UNORDERED_MAP(RepoId, RemoteInfo) RemoteInfoMap;
#else
  typedef NDDS_MAP_CMP(RepoId, RemoteInfo, GUID_tKeyLessThan) RemoteInfoMap;
#endif
  ...
  typedef NDDS_VECTOR(MetaSubmessage) MetaSubmessageVec;
  typedef NDDS_VECTOR(MetaSubmessageVec) MetaSubmessageVecVec;

  // RTPS reliability support for local writers:

  typedef CORBA::ULong FragmentNumberValue;
  typedef NDDS_MAP(FragmentNumberValue, RTPS::FragmentNumberSet) RequestedFragMap;
  typedef NDDS_MAP(SequenceNumber, RequestedFragMap) RequestedFragSeqMap;
  ...
  struct ReaderInfo : public RcObject {
    ...
    NDDS_MAP(SequenceNumber, TransportQueueElement*) durable_data_;
    ...
    {}
    ~ReaderInfo();
    void swap_durable_data(NDDS_MAP(SequenceNumber, TransportQueueElement*)& dd);
...
#ifdef ACE_HAS_CPP11
  typedef NDDS_UNORDERED_MAP(RepoId, ReaderInfo_rch) ReaderInfoMap;
#else
  typedef NDDS_MAP_CMP(RepoId, ReaderInfo_rch, GUID_tKeyLessThan) ReaderInfoMap;
#endif
  typedef NDDS_SET(ReaderInfo_rch) ReaderInfoSet;
  struct ReaderInfoSetHolder : RcObject {
    ReaderInfoSet readers;
  };
  typedef RcHandle<ReaderInfoSetHolder> ReaderInfoSetHolder_rch;
  typedef NDDS_MAP(SequenceNumber, ReaderInfoSetHolder_rch) SNRIS;
  ...
  private:
    ...
    typedef NDDS_MULTISET(NDDS::DCPS::SequenceNumber) SequenceNumberMultiset;
    ...
    typedef NDDS_SET_CMP(TransportQueueElement*, TransportQueueElement::OrderBySequenceNumber) TqeSet;
    typedef NDDS_MULTIMAP(SequenceNumber, TransportQueueElement*) SnToTqeMap;
    ...
    void remove_preassociation_reader(const ReaderInfo_rch& reader)
    {
      if (preassociation_readers_.erase(reader)) {
        ...
        NDDS_ASSERT(pos != preassociation_reader_start_sns_.end());
...
#ifdef ACE_HAS_CPP11
  typedef NDDS_UNORDERED_MAP(RepoId, RtpsWriter_rch) RtpsWriterMap;
#else
  typedef NDDS_MAP_CMP(RepoId, RtpsWriter_rch, GUID_tKeyLessThan) RtpsWriterMap;
#endif
  ...
  struct WriterInfo : RcObject {
    ...
    typedef NDDS_MAP(SequenceNumber, ReceivedDataSample) HeldMap;
    HeldMap held_;
    SequenceNumber hb_last_;
    NDDS_MAP(SequenceNumber, RTPS::FragmentNumber_t) frags_;
...
#ifdef ACE_HAS_CPP11
  typedef NDDS_UNORDERED_MAP(RepoId, WriterInfo_rch) WriterInfoMap;
#else
  typedef NDDS_MAP_CMP(RepoId, WriterInfo_rch, GUID_tKeyLessThan) WriterInfoMap;
#endif
  typedef NDDS_SET(WriterInfo_rch) WriterInfoSet;
  ...
  typedef NDDS_VECTOR(MetaSubmessageVec::iterator) MetaSubmessageIterVec;
#ifdef ACE_HAS_CPP11
  typedef NDDS_UNORDERED_MAP(RepoId, MetaSubmessageIterVec) DestMetaSubmessageMap;
#else
  typedef NDDS_MAP_CMP(RepoId, MetaSubmessageIterVec, GUID_tKeyLessThan) DestMetaSubmessageMap;
#endif
  typedef NDDS_MAP(AddressCacheEntryProxy, DestMetaSubmessageMap) AddrDestMetaSubmessageMap;
  typedef NDDS_VECTOR(MetaSubmessageIterVec) MetaSubmessageIterVecVec;
  typedef NDDS_SET(CORBA::Long) CountSet;
  typedef NDDS_MAP_CMP(EntityId_t, CountSet, EntityId_tKeyLessThan) IdCountSet;
  typedef NDDS_MAP(size_t, IdCountSet) HeartbeatCounts;
  ...
  typedef NDDS_VECTOR(MetaSubmessageVecVec) MetaSubmessageVecVecVec;
  ...
  typedef NDDS_MAP(ACE_thread_t, MetaSubmessageVecVec) ThreadSendQueueMap;
...
#ifdef ACE_HAS_CPP11
  typedef NDDS_UNORDERED_MAP(RepoId, RtpsReader_rch) RtpsReaderMap;
#else
  typedef NDDS_MAP_CMP(RepoId, RtpsReader_rch, GUID_tKeyLessThan) RtpsReaderMap;
#endif
  RtpsReaderMap readers_;

  typedef NDDS_MULTIMAP_CMP(RepoId, RtpsReader_rch, GUID_tKeyLessThan) RtpsReaderMultiMap;
                           ...
                           const FN& func)
  {
    ...
    NDDS_VECTOR(RtpsWriter_rch) to_call;
    {
    ...
    MetaSubmessageVec meta_submessages;
    for (NDDS_VECTOR(RtpsWriter_rch)::const_iterator it = to_call.begin(); it < to_call.end(); ++it) {
  ...
  typedef NDDS_MULTIMAP_CMP(RepoId, InterestingRemote, GUID_tKeyLessThan) InterestingRemoteMapType;
  ...
  typedef NDDS_MAP_CMP(EntityId_t, CORBA::Long, EntityId_tKeyLessThan) CountMapType;
