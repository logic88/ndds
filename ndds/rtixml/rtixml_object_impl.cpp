                               ...
                               const RepoId& remote_id)
{
  ...
  if (pos != locators_.end()) {
    NDDS_ASSERT(pos->second.ref_count_ > 0);
...
RtpsUdpDataLink::RtpsWriter::pre_stop_helper(TqeVector& to_drop, bool true_stop)
{
  ...
  if (!elems_not_acked_.empty()) {
    NDDS_SET(SequenceNumber) sns_to_release;
    ...
    NDDS_SET(SequenceNumber)::iterator sns_it = sns_to_release.begin();
  ...
  bool& deliver_after_send)
{
  ...
  NDDS_ASSERT(element->publication_id() == id_);
  ...
  if (tsce) {        // Control message
    if (RtpsSampleHeader::control_message_supported(tsce->header().message_id_)) {
      ...
      // Create RTPS Submessage(s) in place of the NDDS DataSampleHeader
      ...
      if (ri != remote_readers_.end()) {
        ...
        if (Transport_debug_level > 3) {
          ...
          ACE_DEBUG((LM_DEBUG,
            ...
            NDDS_STRING(conv).c_str(), NDDS_STRING(sub_conv).c_str(),
                          ...
                          const NetworkAddress& remote_addr)
{
  ...
  NDDS_VECTOR(RtpsReader_rch) to_call;
  {
    ACE_GUARD(ACE_Thread_Mutex, g, readers_lock_);
  ...
  MetaSubmessageVec meta_submessages;
  for (NDDS_VECTOR(RtpsReader_rch)::const_iterator it = to_call.begin(); it < to_call.end(); ++it) {
                                           ...
                                           MetaSubmessageVec&)
{
  ...
  const NDDS_VECTOR(SequenceRange) psr = gaps.present_sequence_ranges();
  for (NDDS_VECTOR(SequenceRange)::const_iterator pos = psr.begin(), limit = psr.end(); pos != limit; ++pos) {
                          ...
                          const NetworkAddress& remote_addr)
{
  ...
  MetaSubmessageVec meta_submessages;
  NDDS_VECTOR(InterestingRemote) callbacks;
  {
    ACE_GUARD(ACE_Thread_Mutex, g, readers_lock_);
                                                 ...
                                                 MetaSubmessageVec& meta_submessages)
{
  ...
  if (!(hb_first < 1 || hb_last < 0 || hb_last < hb_first.previous())) {
    if (writer->recvd_.empty() && (directed || !writer->sends_directed_hb())) {
      NDDS_ASSERT(preassociation_writers_.count(writer));
...
{
  ACE_GUARD_RETURN(ACE_Thread_Mutex, g, mutex_, false);
  NDDS_ASSERT(!reader->durable_ || durable_);
...
{
  NDDS_MAP(SequenceNumber, TransportQueueElement*) dd;
  TqeSet to_drop;
  ...
  bool result = false;
  {
    ...
    if (it != remote_readers_.end()) {
      ...
      if (is_pvs_writer_ &&
        ...
        const NDDS_VECTOR(SequenceRange) psr = reader->pvs_outstanding_.present_sequence_ranges();
        for (NDDS_VECTOR(SequenceRange)::const_iterator pos = psr.begin(), limit = psr.end(); pos != limit; ++pos) {
          ACE_GUARD_RETURN(ACE_Thread_Mutex, g, elems_not_acked_mutex_, result);
          for (SequenceNumber seq = pos->first; seq <= pos->second; ++seq) {
            NDDS_MULTIMAP(SequenceNumber, TransportQueueElement*)::iterator iter = elems_not_acked_.find(seq);
  ...
  typedef NDDS_MAP(SequenceNumber, TransportQueueElement*)::iterator iter_t;
                                                             ...
                                                             const WriterInfo_rch& writer)
{
  ...
  NDDS_ASSERT(writer->recvd_.empty());
...
typedef NDDS_VECTOR(size_t) SizeVec;
...
RtpsUdpDataLink::bundle_mapped_meta_submessages(const Encoding& encoding,
                                                ...
                                                NDDS_VECTOR(AddressCacheEntryProxy)& meta_submessage_bundle_addrs,
                                                NDDS_VECTOR(size_t)& meta_submessage_bundle_sizes,
                                                CountKeeper& counts)
{
  using namespace RTPS;
...
RtpsUdpDataLink::bundle_and_send_submessages(MetaSubmessageVecVecVec& meta_submessages)
{
  ...
  NDDS_VECTOR(AddressCacheEntryProxy) meta_submessage_bundle_addrs; // for a bundle's address set
  ...
  NDDS_ASSERT(meta_submessage_bundles.size() == meta_submessage_bundle_addrs.size());
  NDDS_ASSERT(meta_submessage_bundles.size() == meta_submessage_bundle_sizes.size());
...
RtpsUdpDataLink::RtpsReader::generate_nack_frags_i(MetaSubmessageVec& meta_submessages,
                                                   ...
                                                   ACE_CDR::ULong& cumulative_bits_added)
{
  typedef NDDS_MAP(SequenceNumber, RTPS::FragmentNumber_t)::iterator iter_t;
  ...
  NDDS_VECTOR(SequenceRange) missing = wi->recvd_.missing_sequence_ranges();
                          ...
                          const NetworkAddress& remote_addr)
{
  ...
  const MonotonicTimePoint now = MonotonicTimePoint::now();
  NDDS_VECTOR(DiscoveryListener*) callbacks;
                                             ...
                                             MetaSubmessageVec& meta_submessages)
{
  ...
  if (Transport_debug_level > 5) {
    ...
    ACE_DEBUG((LM_DEBUG, "(%P|%t) RtpsUdpDataLink::received(ACKNACK) "
      "local %C remote %C\n", NDDS_STRING(local_conv).c_str(),
      NDDS_STRING(remote_conv).c_str()));
  }
  ...
  NDDS_MAP(SequenceNumber, TransportQueueElement*) pendingCallbacks;

  if (Transport_debug_level > 5) {
    GuidConverter local_conv(id_), remote_conv(src);
    ACE_DEBUG((LM_DEBUG, "(%P|%t) RtpsUdpDataLink::RtpsWriter::process_acknack "
               "local %C remote %C ack %q\n", NDDS_STRING(local_conv).c_str(),
               NDDS_STRING(remote_conv).c_str(), ack.getValue()));
  }
  ...
  if (ack != SequenceNumber::SEQUENCENUMBER_UNKNOWN()) {
    ...
    if (!reader->durable_data_.empty()) {
      ...
      if (ack > dd_last) {
        ...
        for (NDDS_MAP(SequenceNumber, TransportQueueElement*)::iterator pos = reader->durable_data_.begin(),
  ...
  if (is_pvs_writer_ &&
    ...
    const NDDS_VECTOR(SequenceRange) psr = reader->pvs_outstanding_.present_sequence_ranges();
    for (NDDS_VECTOR(SequenceRange)::const_iterator pos = psr.begin(), limit = psr.end();
...
RtpsUdpDataLink::RtpsWriter::gather_nack_replies_i(MetaSubmessageVec& meta_submessages)
{
  ...
  typedef NDDS_MAP(SequenceNumber, DisjointSequence) FragmentInfo;

  // Consolidated non-directed requests and address sets to be sent together at the end, after directed replies
  typedef NDDS_MAP(SequenceNumber, AddrSet) RecipientMap;
  typedef NDDS_MAP(SequenceNumber, RepoIdSet) ReaderMap;
  ...
  for (ReaderInfoSet::const_iterator pos = readers_expecting_data_.begin(), limit = readers_expecting_data_.end();
    ...
    if (reader->expecting_durable_data()) {
      ...
      if (!reader->requests_.empty() && !reader->durable_data_.empty()) {
        ...
        if (reader->requests_.high() < dd_first) {
          ...
          const NDDS_VECTOR(SequenceRange) psr = reader->requests_.present_sequence_ranges();
          for (NDDS_VECTOR(SequenceRange)::const_iterator iter = psr.begin(), limit = psr.end();
               iter != limit && iter->first <= dd_last; ++iter) {
            for (SequenceNumber s = iter->first; s <= iter->second; ++s) {
              if (s <= dd_last) {
                const NDDS_MAP(SequenceNumber, TransportQueueElement*)::iterator dd_iter = reader->durable_data_.find(s);
      ...
      for (rfs_iter rfs = reader->requested_frags_.begin(), limit = reader->requested_frags_.end(); rfs != limit; ++rfs) {
        const NDDS_MAP(SequenceNumber, TransportQueueElement*)::iterator dd_iter = reader->durable_data_.find(rfs->first);
        if (dd_iter != reader->durable_data_.end()) {
    ...
    const NDDS_VECTOR(SequenceRange) ranges = reader->requests_.present_sequence_ranges();
    for (NDDS_VECTOR(SequenceRange)::const_iterator iter = ranges.begin(), limit = ranges.end();
         iter != limit; ++iter) {
...

  {
    ...
    const NDDS_VECTOR(SequenceRange) ranges = consolidated_requests.present_sequence_ranges();
    for (NDDS_VECTOR(SequenceRange)::const_iterator pos = ranges.begin(), limit = ranges.end();
         pos != limit; ++pos) {
      if (Transport_debug_level > 5) {
...
RtpsUdpDataLink::RtpsWriter::check_leader_lagger() const
{
  ...
  for (SNRIS::const_iterator pos1 = lagging_readers_.begin(), limit = lagging_readers_.end();
    ...
    for (ReaderInfoSet::const_iterator pos2 = readers->readers.begin(), limit = readers->readers.end();
      ...
      NDDS_ASSERT(reader->acked_sn() == sn);
      const SequenceNumber expect_max_sn = expected_max_sn(reader);
      NDDS_ASSERT(sn == negative_one || sn < expect_max_sn);
      NDDS_ASSERT(preassociation_readers_.count(reader) == 0);
    }
  }
...
{
  using namespace NDDS::RTPS;
  typedef NDDS_MULTIMAP(SequenceNumber, TransportQueueElement*)::iterator iter_t;
  NDDS_VECTOR(RepoId) to_check;
...
{
  NDDS_VECTOR(CallbackType) readerDoesNotExistCallbacks;
...

  {
    ...
    typedef NDDS_MAP_CMP(RepoId, RepoIdSet, GUID_tKeyLessThan) WtaMap;
  ...
  for (NDDS_VECTOR(CallbackType)::iterator iter = readerDoesNotExistCallbacks.begin();
                                                         ...
                                                         const ReaderInfo_rch& reader)
{
  ...
  NDDS_ASSERT(!(first_sn < 1 || last_sn < 0 || last_sn < first_sn.previous()));
  meta_submessages.push_back(meta_submessage);
...
RtpsUdpDataLink::RtpsWriter::gather_heartbeats(const RepoIdSet& additional_guids,
                                               MetaSubmessageVec& meta_submessages)
{
  NDDS_ASSERT(!additional_guids.empty());
...
{
  NDDS_VECTOR(CallbackType) writerDoesNotExistCallbacks;
  ...
  NDDS_VECTOR(CallbackType)::iterator iter;
...
void
RtpsUdpDataLink::ReaderInfo::swap_durable_data(NDDS_MAP(SequenceNumber, TransportQueueElement*)& dd)
{
...
RtpsUdpDataLink::RtpsReader::deliver_held_data(const RepoId& src)
{
  ...
  NDDS_VECTOR(ReceivedDataSample) to_deliver;
  ...
  for (NDDS_VECTOR(ReceivedDataSample)::iterator it = to_deliver.begin(); it != to_deliver.end(); ++it) {
    if (Transport_debug_level > 5) {
...
{
  NDDS_ASSERT(local != GUID_UNKNOWN);
  NDDS_ASSERT(remote != GUID_UNKNOWN);
