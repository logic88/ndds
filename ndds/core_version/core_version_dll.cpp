...
const char* SecurityRegistry::DEFAULT_CONFIG_NAME = "_NDDS_DEFAULT_CONFIG";
const char* SecurityRegistry::BUILTIN_CONFIG_NAME = "_NDDS_BUILTIN_CONFIG";
const char* SecurityRegistry::DEFAULT_INST_PREFIX = "_NDDS_";
...
SecurityRegistry::SecurityConfigEntry::SecurityConfigEntry(const NDDS_STRING& entryName)
...
void
SecurityRegistry::SecurityConfigEntry::add_property(const NDDS_STRING& name, const NDDS_STRING& value)
{
  // Move these up
...
{
  DBG_ENTRY_LVL("SecurityRegistry", "SecurityRegistry", 6);
  lib_directive_map_[DEFAULT_PLUGIN_NAME] = "dynamic NDDS_Security Service_Object * NDDS_Security:_make_BuiltInPluginLoader()";
}
...
void
SecurityRegistry::register_plugin(const NDDS_STRING& plugin_name,
...
SecurityConfig_rch
SecurityRegistry::create_config(const NDDS_STRING& config_name)
{
...
SecurityConfig_rch
SecurityRegistry::get_config(const NDDS_STRING& config_name) const
{
  GuardType guard(lock_);
...
SecurityRegistry::builtin_config() const
{
  ...
  if (!builtin_config_) {
    ...
    NDDS_ASSERT(lib_iter != lib_directive_map_.end());
...
void
SecurityRegistry::bind_config(const NDDS_STRING& name,
  ...
  for (int index = 0; cf.enumerate_sections(root, index, sect_name) == 0; ++index) {
    if (ACE_OS::strcmp(sect_name.c_str(), ACE_TEXT_CHAR_TO_TCHAR(SECURITY_SECTION_NAME)) == 0) {
      ...
      for (DCPS::KeyList::const_iterator it = keys.begin(); it != keys.end(); ++it) {
        const NDDS_STRING& entry_name = it->first;
...
void
SecurityRegistry::load_security_plugin_lib(const NDDS_STRING& security_plugin_type)
{
...
bool
SecurityRegistry::find_config(const NDDS_STRING& name, SecurityConfig_rch& config)
{
  GuardType guard(lock_);
...
bool
SecurityRegistry::add_config(const NDDS_STRING& name, SecurityConfig_rch& config)
{
  GuardType guard(lock_);
...
SecurityPluginInst_rch SecurityRegistry::get_plugin_inst(
  const NDDS_STRING& plugin_name, bool attempt_fix)
{
  GuardType guard(lock_);
