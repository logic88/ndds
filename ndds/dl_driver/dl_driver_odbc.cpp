  ...
  if (!this->servant_finalized_) {
    ...
    if (shutdown_error) {
      ACE_ERROR((LM_ERROR, "(%P|%t) ERROR: InfoRepo::finalize: "
        ...
        NDDS::DCPS::retcode_to_string(shutdown_error)));
    }
...
InfoRepo::init()
{
  ...
  NDDS::DCPS::DCPSInfo_var info_repo =
    NDDS::DCPS::DCPSInfo::_unchecked_narrow(obj);
  ...
  NDDS::DCPS::Service_Participant* serv_part = TheServiceParticipant;
  serv_part->set_repo_ior(objref_str, NDDS::DCPS::Discovery::DEFAULT_REPO);

  NDDS::DCPS::Discovery_rch disc = serv_part->get_discovery(0 /*domainId*/);
  NDDS::DCPS::InfoRepoDiscovery_rch ird =
    NDDS::DCPS::static_rchandle_cast<NDDS::DCPS::InfoRepoDiscovery>(disc);
  if (!ird->set_ORB(orb_)) {
    ACE_ERROR((LM_ERROR, ACE_TEXT("(%P|%t) ERROR: DCPSInfoRepo::init: ")
  ...
  NDDS::Federator::Manager_var federator;
  ...
  if (federator_.id().overridden()) {
    ...
    obj = info_poa->id_to_reference(oid);
    federator = NDDS::Federator::Manager::_narrow(obj);
  ...
  if (CORBA::is_nil(adapter)) {
    ...
    adapter->bind(NDDS::Federator::REPOSITORY_IORTABLE_KEY, objref_str);

    if (this->federator_.id().overridden()) {
      // Bind to '/Federator'
      adapter->bind(NDDS::Federator::FEDERATOR_IORTABLE_KEY, federator_ior);

      // Bind to '/Federator/1382379631'
      std::stringstream buffer(NDDS::Federator::FEDERATOR_IORTABLE_KEY);
  ...
  if (this->federator_.id().overridden()
    ...
    NDDS::Federator::Manager_var peer =
      NDDS::Federator::Manager::_narrow(obj);
