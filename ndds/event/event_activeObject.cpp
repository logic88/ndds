...
Manager::pushImage(const DImage& image)
{
  ...
  ***************************/
  ...
  SeqGuard<NDDS::DCPS::TransportLocatorSeq> trans_guard;
  SeqGuard<NDDS::DCPS::TransportLocatorSeq>::Seq& transports = trans_guard.seq();
  ...
  for (DImage::ReaderSeq::const_iterator iter = image.actors.begin();
    ...
    NDDS::DCPS::TransportLocatorSeq* trans;
    ACE_NEW_NORETURN(trans, NDDS::DCPS::TransportLocatorSeq);
...
Manager::add(const DActor& actor)
{
  ...
  NDDS::DCPS::TransportLocatorSeq transport_info;
