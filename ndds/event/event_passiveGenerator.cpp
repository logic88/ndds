...
namespace {
  ...
  NDDS::DCPS::RepoId build_id(const ConnectionSettings& conn)
  {
    ...
    return NDDS::DCPS::EndpointRegistry::build_id(conn.domain_id_,
                                                     participant_key,
                                                     NDDS::DCPS::EndpointRegistry::build_id(entity_key,
                                                                                               entity_kind));
  }
...
Parser::parse(const char* filename)
{
  ...
  for (ConnectionMap::const_iterator pos = connection_map_.begin(), limit = connection_map_.end();
    ...
    NDDS::DCPS::RepoId id = build_id(conn);

    switch (conn.direction_) {
    case FACE::SOURCE:
      {
        NDDS_STRING topic_name = conn.topic_name_;
        ...
        NDDS::DCPS::EndpointRegistry::Writer w(topic_name, qos, publisher_qos, conn.config_name(), trans_info);
        NDDS::DCPS::StaticDiscovery::instance()->registry.writer_map.insert(std::make_pair(id, w));
      }
      break;
    case FACE::DESTINATION:
      {
        ...
        NDDS::DCPS::EndpointRegistry::Reader r(topic_name, qos, subscriber_qos, conn.config_name(), trans_info);
        NDDS::DCPS::StaticDiscovery::instance()->registry.reader_map.insert(std::make_pair(id, r));
      }
      break;
    case FACE::BI_DIRECTIONAL:
    case FACE::ONE_WAY_REQUEST_SOURCE:
