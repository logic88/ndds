...
ManagerImpl::initialize()
{
  ...
  std::string inst_name = NDDS::DCPS::TransportRegistry::DEFAULT_INST_PREFIX
  ...
  } else if (NDDS::DCPS::DCPS_debug_level > 4) {
    ACE_DEBUG((LM_DEBUG,
  ...
  try {
    NDDS::DCPS::TransportRegistry::instance()->bind_config(config,
                                                              subscriber.in());
  ...
  if (::CORBA::is_nil(this->ownerWriter_.in())) {
    ...
    NDDS::DCPS::DataWriterImpl* servant
    = dynamic_cast<NDDS::DCPS::DataWriterImpl*>(dataWriter.in());
    ...
    if (0 == servant) {
      ...
      NDDS::DCPS::RepoIdConverter converter(servant->get_repo_id());
      ACE_DEBUG((LM_DEBUG,
                 ACE_TEXT("(%P|%t) Federator::ManagerImpl::initialize() - ")
  ...
  if (CORBA::is_nil(dataReader.in())) {
    ...
    NDDS::DCPS::DataReaderImpl* servant
    = dynamic_cast<NDDS::DCPS::DataReaderImpl*>(dataReader.in());
  ...
  if (this->multicastEnabled_) {
    ...
    const char *port_number = ACE_OS::getenv("NDDSFederationPort");
    ...
    // use the default.
    if (port == 0)
      port = NDDS::Federator::Defaults::DiscoveryRequestPort;
...
ManagerImpl::repository()
{
  ...
  NDDS::DCPS::Discovery_rch disco
  = TheServiceParticipant->get_discovery(
      this->config_.federationDomain());
  NDDS::DCPS::DCPSInfo_var repo;
  if (!disco.is_nil()) {
    NDDS::DCPS::InfoRepoDiscovery_rch irDisco =
      DCPS::static_rchandle_cast<DCPS::InfoRepoDiscovery>(disco);
    repo = irDisco->get_dcps_info();
  }

  if (CORBA::is_nil(repo.in())) {
    return NDDS::DCPS::DCPSInfo::_duplicate(this->localRepo_.in());

  } else {
    return NDDS::DCPS::DCPSInfo::_duplicate(repo.in());
  }
}
...
)
{
  ...
  if (this->federated_ == false) {
    ...
    try {
      ...
      NDDS::DCPS::DCPSInfo_var remoteRepo = peer->repository();
  ...
  try {
    ...
    if (this->joinRepo_ == remote) {
      ...
      = NDDS::Federator::Manager::_duplicate(remoteManager.in());
    }
...
ManagerImpl::initializeOwner(
  const NDDS::Federator::OwnerUpdate & data)
{
...
ManagerImpl::initializeTopic(
  const NDDS::Federator::TopicUpdate & data)
{
  this->processCreate(&data, 0);
}
...
ManagerImpl::initializeParticipant(
  const NDDS::Federator::ParticipantUpdate & data)
{
  this->processCreate(&data, 0);
}
...
ManagerImpl::initializePublication(
  const NDDS::Federator::PublicationUpdate & data)
{
  this->processCreate(&data, 0);
}
...
ManagerImpl::initializeSubscription(
  const NDDS::Federator::SubscriptionUpdate & data)
{
  this->processCreate(&data, 0);
}
