...
#ifndef NDDS_DCPS_TRANSPORT_FRAMEWORK_TRANSPORTSTATISTICS_H
#define NDDS_DCPS_TRANSPORT_FRAMEWORK_TRANSPORTSTATISTICS_H
...
struct InternalTransportStatistics {
  const NDDS_STRING transport;
  typedef NDDS_MAP(InternalMessageCountKey, InternalMessageCountValue) MessageCountMap;
  MessageCountMap message_count;
  typedef NDDS_MAP(GUID_t, CORBA::ULong) GuidCountMap;
  GuidCountMap writer_resend_count;
  GuidCountMap reader_nack_count;

  explicit InternalTransportStatistics(const NDDS_STRING& a_transport)
...
#endif  /* NDDS_DCPS_TRANSPORTSTATISTICS_H */
