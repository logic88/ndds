...
bool NetworkInterfaceAddress::exclude_from_multicast(const char* configured_interface) const
{
  ...
  if (configured_interface && *configured_interface && name != configured_interface) {
    NDDS_STRING ci_semi(configured_interface);
    if (ci_semi.find_first_of(':') == NDDS_STRING::npos) {
      ci_semi += ':';
    }
...
void NetworkConfigMonitor::remove_interface(const NDDS_STRING& name)
{
  ACE_GUARD(ACE_Thread_Mutex, g, mutex_);
...
void NetworkConfigMonitor::remove_address(const NDDS_STRING& name, const NetworkAddress& address)
{
  ACE_GUARD(ACE_Thread_Mutex, g, mutex_);
