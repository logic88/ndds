...
#ifndef NDDS_IDL_BE_GLOBAL_H
#define NDDS_IDL_BE_GLOBAL_H
...
public:
  ...
  /// Called to indicate that NDDS marshaling (serialization) code for the
  ...
  NDDS::DataRepresentation data_representations(AST_Decl* node) const;

  NDDS::XTypes::MemberId compute_id(AST_Field* field, AutoidKind auto_id, NDDS::XTypes::MemberId& member_id);
  NDDS::XTypes::MemberId get_id(AST_Field* field);
...
private:
  ...
  NDDS::DataRepresentation default_data_representation_;
  AutoidKind root_default_autoid_;
  TryConstructFailAction default_try_construct_;
  std::set<std::string> platforms_;
  typedef std::map<AST_Field*, NDDS::XTypes::MemberId> MemberIdMap;
