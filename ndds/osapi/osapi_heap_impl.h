...
#ifndef NDDS_DCPS_MULTITOPICDATAREADERBASE_H
#define NDDS_DCPS_MULTITOPICDATAREADERBASE_H
...
class NDDS_Dcps_Export MultiTopicDataReaderBase
...
public:
  ...
  // NDDS::DCPS::DataReaderEx interface
...
private:
  ...
  unique_ptr<NDDS::DCPS::LocalObject<DDS::DataReaderListener> > listener_;
  DataReaderEx_var resulting_reader_;

protected:

  NDDS_STRING topicNameFor(DDS::DataReader_ptr dr);
  ...
  struct QueryPlan {
    ...
    std::vector<NDDS_STRING> keys_projected_out_;
    std::multimap<NDDS_STRING, NDDS_STRING> adjacent_joins_; // topic -> key
  ...
  NDDS_MAP(NDDS_STRING, QueryPlan) query_plans_;

  NDDS_DELETED_COPY_MOVE_CTOR_ASSIGN(MultiTopicDataReaderBase)
};
