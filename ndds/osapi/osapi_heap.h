...
#ifndef NDDS_DCPS_MULTITOPICDATAREADER_T_H
#define NDDS_DCPS_MULTITOPICDATAREADER_T_H
...
private:
  ...
  struct SampleWithInfo {
    SampleWithInfo(const NDDS_STRING& topic, const DDS::SampleInfo& sampinfo)
      : sample_(),
    ...
    NDDS_MAP(NDDS_STRING/*topicName*/, DDS::InstanceHandle_t) info_;
  };

  typedef std::vector<SampleWithInfo> SampleVec;
  typedef std::set<NDDS_STRING> TopicSet;
  ...
  DDS::ReturnCode_t process_joins(NDDS_MAP(TopicSet, SampleVec)& partialResults,
  ...
  DDS::ReturnCode_t join(SampleVec& resulting, const SampleWithInfo& prototype,
                         const std::vector<NDDS_STRING>& key_names,
  ...
  // far (partialResults).
  DDS::ReturnCode_t cross_join(NDDS_MAP(TopicSet, SampleVec)& partialResults,
  ...
  struct Contains { // predicate for std::find_if()
    ...
    explicit Contains(const NDDS_STRING& s) : look_for_(s) {}
    bool operator()(const std::pair<const std::set<NDDS_STRING>, SampleVec>& e)
      const {
