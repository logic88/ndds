...
#ifndef NDDS_DCPS_MULTITOPICDATAREADER_T_CPP
#define NDDS_DCPS_MULTITOPICDATAREADER_T_CPP
  ...
  const MetaStruct& meta)
{
  ...
  const vector<NDDS_STRING>& proj_out = qp.keys_projected_out_;
  for (vector<NDDS_STRING>::const_iterator iter = proj_out.begin();
...
MultiTopicDataReader_T<Sample, TypedDataReader>::join(
  ...
  const std::vector<NDDS_STRING>& key_names, const void* key_data,
  ...
  if (n_keys > 0 && other_meta.numDcpsKeys() == n_keys) { // complete key
    ...
    if (ih != HANDLE_NIL) {
      ...
      if (ret != RETCODE_OK && ret != RETCODE_NO_DATA) {
        throw std::runtime_error(
         NDDS_STRING("In join(), incoming DataReader for ") + NDDS_STRING(other_topic) +
    ...
    for (InstanceHandle_t ih = HANDLE_NIL; ret != RETCODE_NO_DATA;) {
      ...
      if (ret != RETCODE_OK && ret != RETCODE_NO_DATA) {
        ...
        ss
          << "In join(), incoming DataReader for " << NDDS_STRING(other_topic)
...
MultiTopicDataReader_T<Sample, TypedDataReader>::combine(
  ...
  const std::vector<NDDS_STRING>& key_names, const TopicSet& other_topics)
{
  const MetaStruct& meta = getResultingMeta();
  SampleVec newData;
  ...
  const TopicSet& seen, const QueryPlan& qp)
{
  ...
  typedef multimap<NDDS_STRING, NDDS_STRING>::const_iterator iter_t;
  for (iter_t iter = qp.adjacent_joins_.begin();
       iter != qp.adjacent_joins_.end();) { // for each topic we're joining
    const NDDS_STRING& other_topic = iter->first;
    ...
    try {
      ...
      if (found == partialResults.end()) { // haven't seen this topic yet
        ...
        for (set<NDDS_STRING>::const_iterator i3 = found->first.begin();
  ...
  const DDS::SampleInfo& info, const char* topic, const MetaStruct& meta)
{
  ...
  for (std::map<NDDS_STRING, QueryPlan>::iterator iter = query_plans_.begin();
  ...
  for (typename std::map<TopicSet, SampleVec>::iterator iterPR =
    ...
    for (typename SampleVec::iterator i = iterPR->second.begin();
      ...
      if (ih != HANDLE_NIL) {
        typedef std::map<NDDS_STRING, InstanceHandle_t>::iterator mapiter_t;
