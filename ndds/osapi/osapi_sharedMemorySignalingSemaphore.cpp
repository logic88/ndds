...
#define NDDS_IDL_STR(X) #X

using std::string;
using namespace AstTypeClassification;
using NDDS::DCPS::array_count;

namespace {
  const string RtpsNamespace = " ::NDDS::RTPS::", DdsNamespace = " ::DDS::";
  ...
  // TODO(iguessthislldo): Replace with Encoding::Kind from libNDDS_Util. See XTYPE-140
  enum Encoding {
...
namespace {
  ...
  {
    {
      ...
      be_global->impl_ <<
        ...
        "    if (seq[i]._d() == NDDS::RTPS::PID_SENTINEL) continue;\n"
    ...
    }
    {
      ...
      be_global->impl_ <<
        ...
        "    }\n"
        "  }\n"
        "  return (strm << NDDS::RTPS::PID_SENTINEL)\n"
        "    && (strm << NDDS::RTPS::PID_PAD);\n";
    }
    {
      Function extraction("operator>>", "bool");
      extraction.addArg("strm", "Serializer&");
      ...
      be_global->impl_ <<
        "  while (true) {\n"
        "    const CORBA::ULong idx = NDDS::DCPS::grow(seq) - 1;\n"
        "    if (!(strm >> seq[idx])) {\n"
        "      return false;\n"
        "    }\n"
        "    if (seq[idx]._d() == NDDS::RTPS::PID_SENTINEL) {\n"
        "      seq.length(idx);\n"
        "      return true;\n"
        "    }\n"
        "  }\n";
   ...
   * NDDS::DCPS::Encoding::XcdrVersion.
   */
  ...
  // TODO(iguessthislldo): Replace with align from libNDDS_Util. See XTYPE-140
    ...
    Encoding encoding, size_t& size, AST_Type* type, size_t n)
  {
    ...
    if (n > 1) {
      ...
      // TODO(iguessthislldo): https://github.com/objectcomputing/NDDS/pull/1668#discussion_r432521888
...
namespace {
    ...
    if ((fld_cls & CL_STRING) && (dir == WD_INPUT)) {
      ...
      if (dir == WD_INPUT) { // no need to warn twice
        std::cerr << "WARNING: field " << name << " can not be serialized.  "
          ...
          ") can not be used in an NDDS topic type." << std::endl;
      }
      return "false";
  ...
  std::string fill_datareprseq(
    const NDDS::DataRepresentation& repr,
    ...
    const std::string& indent)
  {
    ...
    if (repr.unaligned) {
      values.push_back("NDDS::DCPS::UNALIGNED_CDR_DATA_REPRESENTATION");
    }
...

    {
      ...
      if (is_mutable) {
        ...
        */
        ...
        for (Fields::Iterator i = fields.begin(); i != fields_end; ++i) {
          AST_Field* const field = *i;
          const NDDS::XTypes::MemberId id = be_global->get_id(field);
          string field_name =
                                   ...
                                   const char* /* repoid */)
{
  ...
  if (be_global->printer()) {
    ...
    PreprocessorIfGuard g("ndef NDDS_SAFETY_PROFILE");
...
marshal_generator::gen_field_getValueFromSerialized(AST_Structure* node, const std::string& clazz)
{
  ...
  if (is_mutable) {
    be_global->impl_ <<
      ...
      "        bool must_understand = false;\n"
      "        if (!strm.read_parameter_id(member_id, field_size, must_understand)) {\n"
      "          throw std::runtime_error(\"Field \" + NDDS_STRING(field) + \" Deserialization "
      "Error for struct " << clazz << "\");\n"
      "        }\n"
      "        if (encoding.xcdr_version() == Encoding::XCDR_VERSION_1 &&\n"
      "            member_id == Serializer::pid_list_end) {\n"
      "          throw std::runtime_error(\"Field \" + NDDS_STRING(field) + \" not "
  ...
  for (Fields::Iterator i = fields.begin(); i != fields_end; ++i) {
    ...
    if (fld_cls & CL_SCALAR) {
      ...
      if (fld_cls & CL_STRING) {
        ...
        expr +=
          "      if (!strm.skip(1,  " + NDDS::DCPS::to_dds_string(size) + " )) {\n"
      ...
      expr +=
        ...
        "      throw std::runtime_error(\"Field \" + NDDS_STRING(field) + \" could not be skipped\");\n"
        "    }\n";
    }
  }
  be_global->impl_ <<
...
namespace {
                        ...
                        const std::vector<AST_UnionBranch*>& branches)
  {
    ...
    }
    {
      ...
      be_global->impl_ <<
        ...
        "    return false;\n"
        "  }\n"
        "  if (disc == NDDS::RTPS::PID_SENTINEL) {\n"
        "    uni._d(NDDS::RTPS::PID_SENTINEL);\n"
        "    return true;\n"
        "  }\n"
        "  if (size == 0) {\n"
        "    uni._d(disc);\n"
  ...
  void gen_union_key_serializers(AST_Union* node, FieldFilter kind)
  {
    ...
    const string wrapper = kind == FieldFilter_KeyOnly ? "KeyOnly"
      ...
      : "<<error from " __FILE__ ":" NDDS_IDL_STR(__LINE__) ">>";
