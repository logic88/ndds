...
namespace {
  struct MatchesIncomingName { // predicate for std::find_if()
    const NDDS_STRING& look_for_;
    explicit MatchesIncomingName(const NDDS_STRING& s) : look_for_(s) {}
    bool operator()(const NDDS::DCPS::MultiTopicImpl::SubjectFieldSpec& sfs)
      const {
  ...
  class Listener
    : public virtual NDDS::DCPS::LocalObject<DDS::DataReaderListener> {
  public:
    explicit Listener(NDDS::DCPS::MultiTopicDataReaderBase* outer)
      : outer_(outer)
    {}
  ...
  private:
    NDDS::DCPS::MultiTopicDataReaderBase* outer_;
  };
}
  ...
  SubscriberImpl* parent, MultiTopicImpl* multitopic)
{
  ...
  std::map<NDDS_STRING, NDDS_STRING> fieldToTopic;

  // key: name of field that's a key for the 'join'
  // mapped: set of topicNames that have this key in common
  std::map<NDDS_STRING, set<NDDS_STRING> > joinKeys;

  listener_.reset(new Listener(this));

  const vector<NDDS_STRING>& selection = multitopic->get_selection();
    ...
    try {
      ...
      for (const char** names = meta.getFieldNames(); *names; ++names) {
        if (fieldToTopic.count(*names)) { // already seen this field name
          set<NDDS_STRING>& topics = joinKeys[*names];
  ...
  if (aggregation.size() == 0) { // "SELECT * FROM ..."
    ...
    for (const char** names = meta.getFieldNames(); *names; ++names) {
      std::map<NDDS_STRING, NDDS_STRING>::const_iterator found =
  ...
  typedef std::map<NDDS_STRING, set<NDDS_STRING> >::const_iterator iter_t;
  for (iter_t iter = joinKeys.begin(); iter != joinKeys.end(); ++iter) {
    const NDDS_STRING& field = iter->first;
    const set<NDDS_STRING>& topics = iter->second;
    for (set<NDDS_STRING>::const_iterator iter2 = topics.begin();
         iter2 != topics.end(); ++iter2) {
      const NDDS_STRING& topic = *iter2;
      ...
      for (set<NDDS_STRING>::const_iterator iter3 = topics.begin();
           iter3 != topics.end(); ++iter3) {
        if (topic != *iter3) { // other topics
          qp.adjacent_joins_.insert(pair<const NDDS_STRING, NDDS_STRING>(*iter3, field));
        }
      }
    }
  }
...
NDDS_STRING MultiTopicDataReaderBase::topicNameFor(DDS::DataReader_ptr reader)
{
...
void MultiTopicDataReaderBase::data_available(DDS::DataReader_ptr reader)
{
  ...
  const NDDS_STRING topic = topicNameFor(reader);
...
void MultiTopicDataReaderBase::cleanup()
{
  ...
  for (std::map<NDDS_STRING, QueryPlan>::iterator it = query_plans_.begin();
...
#ifndef NDDS_NO_QUERY_CONDITION
