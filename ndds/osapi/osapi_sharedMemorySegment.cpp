...
NDDS::DataRepresentation BE_GlobalData::data_representations(
  AST_Decl* node) const
{
  using namespace NDDS;
  DataRepresentationAnnotation* data_representation_annotation =
    dynamic_cast<DataRepresentationAnnotation*>(
      builtin_annotations_["::NDDS::@data_representation"]);
...
NDDS::XTypes::MemberId BE_GlobalData::compute_id(AST_Field* field, AutoidKind auto_id, NDDS::XTypes::MemberId& member_id)
{
  ...
  using NDDS::XTypes::hash_member_name_to_id;
  const string field_name = field->local_name()->get_string();
  string hash_id;
  NDDS::XTypes::MemberId mid;
  ...
  if (mid > NDDS::DCPS::Serializer::MEMBER_ID_MAX) {
...
NDDS::XTypes::MemberId BE_GlobalData::get_id(AST_Field* field)
{
  const MemberIdMap::const_iterator pos = member_id_map_.find(field);
  if (pos != member_id_map_.end()) {
    return pos->second;
