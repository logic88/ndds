...
ACE_CDR::Boolean
operator<< (ACE_OutputCDR& outCdr, NDDS::DCPS::NetworkResource& value)
{
...
ACE_CDR::Boolean
operator>> (ACE_InputCDR& inCdr, NDDS::DCPS::NetworkResource& value)
{
...
String get_fully_qualified_hostname(ACE_INET_Addr* addr)
{
  ...
  if (fullname.length() == 0) {
    ...
    NDDS::DCPS::HostnameInfoVector nonFQDN;
    ...
    NDDS::DCPS::HostnameInfoVector::iterator itBegin = nonFQDN.begin();
    NDDS::DCPS::HostnameInfoVector::iterator itEnd = nonFQDN.end();

    for (NDDS::DCPS::HostnameInfoVector::iterator it = itBegin; it != itEnd; ++it) {
...
void get_interface_addrs(NDDS_VECTOR(ACE_INET_Addr)& addrs)
{
...
// Check that ACE_INET_Addr supports next()
  ...
  NDDS_VECTOR(ACE_INET_Addr) addresses;
  do {
    ACE_INET_Addr temp;
...
ACE_INET_Addr choose_single_coherent_address(const NDDS_VECTOR(ACE_INET_Addr)& addresses, bool prefer_loopback, const String& name)
{
#ifdef ACE_HAS_IPV6
  NDDS_SET(ACE_INET_Addr) set6_loopback;
  NDDS_SET(ACE_INET_Addr) set6_linklocal;
  NDDS_SET(ACE_INET_Addr) set6_mapped_v4;
  NDDS_SET(ACE_INET_Addr) set6;
#endif // ACE_HAS_IPV6
  NDDS_SET(ACE_INET_Addr) set4_loopback;
  NDDS_SET(ACE_INET_Addr) set4;

  for (NDDS_VECTOR(ACE_INET_Addr)::const_iterator it = addresses.begin(); it != addresses.end(); ++it) {
#ifdef ACE_HAS_IPV6
...
ACE_INET_Addr choose_single_coherent_address(const String& address, bool prefer_loopback, bool allow_ipv4_fallback)
{
  ...
  typedef std::pair<SystemTimePoint, NDDS_SET(ACE_INET_Addr)> AddrCachePair;
  typedef NDDS_MAP(String, AddrCachePair) AddrCacheMap;
