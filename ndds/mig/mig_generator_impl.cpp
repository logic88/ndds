...
DCPS_IR_Participant::DCPS_IR_Participant(const TAO_DDS_DCPSFederationId& federationId,
  ...
  : id_(id),
    ...
    topicIdGenerator_(
      federationId.id(),
      NDDS::DCPS::RepoIdConverter(id).participantId(),
      isBit ? NDDS::DCPS::KIND_BUILTIN_TOPIC : NDDS::DCPS::KIND_USER_TOPIC),
    publicationIdGenerator_(
      federationId.id(),
      NDDS::DCPS::RepoIdConverter(id).participantId(),
      isBit ? NDDS::DCPS::KIND_BUILTIN_WRITER : NDDS::DCPS::KIND_USER_WRITER),
    subscriptionIdGenerator_(
      federationId.id(),
      NDDS::DCPS::RepoIdConverter(id).participantId(),
      isBit ? NDDS::DCPS::KIND_BUILTIN_READER : NDDS::DCPS::KIND_USER_READER),
    um_(um),
...
DCPS_IR_Participant::takeOwnership()
{
  ...
  if (this->um_ && (this->isBitPublisher() == false)) {
    ...
    if (NDDS::DCPS::DCPS_debug_level > 4) {
      NDDS::DCPS::RepoIdConverter converter(id_);
      ACE_DEBUG((LM_DEBUG,
...
int DCPS_IR_Participant::add_publication(NDDS::DCPS::unique_ptr<DCPS_IR_Publication> pub)
{
  NDDS::DCPS::RepoId pubId = pub->get_id();
  ...
  if (where == this->publications_.end()) {
    ...
    this->publications_.insert(
      where, DCPS_IR_Publication_Map::value_type(pubId, NDDS::DCPS::move(pub)));
    ...
    if (NDDS::DCPS::DCPS_debug_level > 0) {
      NDDS::DCPS::RepoIdConverter part_converter(id_);
      NDDS::DCPS::RepoIdConverter pub_converter(pubId);
      ACE_DEBUG((LM_DEBUG,
...
int DCPS_IR_Participant::find_publication_reference(NDDS::DCPS::RepoId pubId,
...
int DCPS_IR_Participant::remove_publication(NDDS::DCPS::RepoId pubId)
{
  DCPS_IR_Publication_Map::iterator where = this->publications_.find(pubId);
...
int DCPS_IR_Participant::add_subscription(NDDS::DCPS::unique_ptr<DCPS_IR_Subscription> sub)
{
  NDDS::DCPS::RepoId subId = sub->get_id();
  ...
  if (where == this->subscriptions_.end()) {
    ...
    this->subscriptions_.insert(
      where, DCPS_IR_Subscription_Map::value_type(subId, NDDS::DCPS::move(sub)));
    ...
    if (NDDS::DCPS::DCPS_debug_level > 0) {
      NDDS::DCPS::RepoIdConverter part_converter(id_);
      NDDS::DCPS::RepoIdConverter sub_converter(subId);
      ACE_DEBUG((LM_DEBUG,
...
int DCPS_IR_Participant::find_subscription_reference(NDDS::DCPS::RepoId subId,
...
int DCPS_IR_Participant::remove_subscription(NDDS::DCPS::RepoId subId)
{
  DCPS_IR_Subscription_Map::iterator where = this->subscriptions_.find(subId);
...
int DCPS_IR_Participant::add_topic_reference(DCPS_IR_Topic* topic)
{
  ...
  if (where == this->topicRefs_.end()) {
    ...
    if (NDDS::DCPS::DCPS_debug_level > 0) {
      NDDS::DCPS::RepoIdConverter part_converter(id_);
      NDDS::DCPS::RepoIdConverter topic_converter(topicId);
      ACE_DEBUG((LM_DEBUG,
...
int DCPS_IR_Participant::remove_topic_reference(NDDS::DCPS::RepoId topicId,
                                                DCPS_IR_Topic*& topic)
{
  DCPS_IR_Topic_Map::iterator where = this->topicRefs_.find(topicId);
...
int DCPS_IR_Participant::find_topic_reference(NDDS::DCPS::RepoId topicId,
                                              DCPS_IR_Topic*& topic)
{
  DCPS_IR_Topic_Map::iterator where = this->topicRefs_.find(topicId);
...

  {
    ...
    while (next != this->topicRefs_.end()) {
      ...
      if (this->um_ && (this->isBitPublisher() == false)) {
        ...
        if (NDDS::DCPS::DCPS_debug_level > 4) {
          NDDS::DCPS::RepoId id = current->second->get_id();
          NDDS::DCPS::RepoIdConverter converter(id);
          ACE_DEBUG((LM_DEBUG,
        ...
        NDDS::DCPS::RepoId id = current->first;
        DCPS_IR_Topic* topic = current->second;

        this->domain_->remove_topic(this, topic);

        if (NDDS::DCPS::DCPS_debug_level > 9) {
          NDDS::DCPS::RepoIdConverter part_converter(id_);
          NDDS::DCPS::RepoIdConverter topic_converter(id);
          ACE_DEBUG((LM_DEBUG,
                     ACE_TEXT("(%P|%t) DCPS_IR_Participant::remove_all_dependents: ")
...
{
  aliveStatus_ = 0;
  domain_->add_dead_participant(NDDS::DCPS::rchandle_from(this));
}

NDDS::DCPS::RepoId DCPS_IR_Participant::get_id()
{
  return id_;
}
...
void DCPS_IR_Participant::ignore_participant(NDDS::DCPS::RepoId id)
{
  if (NDDS::DCPS::DCPS_debug_level > 0) {
    NDDS::DCPS::RepoIdConverter part_converter(id_);
    NDDS::DCPS::RepoIdConverter ignore_converter(id);
    ACE_DEBUG((LM_DEBUG,
...
void DCPS_IR_Participant::ignore_topic(NDDS::DCPS::RepoId id)
{
  if (NDDS::DCPS::DCPS_debug_level > 0) {
    NDDS::DCPS::RepoIdConverter part_converter(id_);
    NDDS::DCPS::RepoIdConverter ignore_converter(id);
...
void DCPS_IR_Participant::ignore_publication(NDDS::DCPS::RepoId id)
{
  if (NDDS::DCPS::DCPS_debug_level > 0) {
    NDDS::DCPS::RepoIdConverter part_converter(id_);
    NDDS::DCPS::RepoIdConverter ignore_converter(id);
...
void DCPS_IR_Participant::ignore_subscription(NDDS::DCPS::RepoId id)
{
  if (NDDS::DCPS::DCPS_debug_level > 0) {
    NDDS::DCPS::RepoIdConverter part_converter(id_);
    NDDS::DCPS::RepoIdConverter ignore_converter(id);
...
CORBA::Boolean DCPS_IR_Participant::is_participant_ignored(NDDS::DCPS::RepoId id)
{
  return (0 == ignoredParticipants_.find(id));
}

CORBA::Boolean DCPS_IR_Participant::is_topic_ignored(NDDS::DCPS::RepoId id)
{
  return (0 == ignoredTopics_.find(id));
}

CORBA::Boolean DCPS_IR_Participant::is_publication_ignored(NDDS::DCPS::RepoId id)
{
  return (0 == ignoredPublications_.find(id));
}

CORBA::Boolean DCPS_IR_Participant::is_subscription_ignored(NDDS::DCPS::RepoId id)
{
...
DCPS_IR_Participant::dump_to_string(const std::string& prefix, int depth) const
{
  ...
  NDDS::DCPS::RepoIdConverter local_converter(id_);
  ...
  for (TAO_DDS_RepoId_Set::const_iterator ipart = ignoredParticipants_.begin();
       ...
       ipart++)
  {
    NDDS::DCPS::RepoIdConverter ipart_converter(*ipart);
  ...
  for (TAO_DDS_RepoId_Set::const_iterator itop = ignoredTopics_.begin();
       ...
       itop++)
  {
    NDDS::DCPS::RepoIdConverter itop_converter(*itop);
  ...
  for (TAO_DDS_RepoId_Set::const_iterator ipub = ignoredPublications_.begin();
       ...
       ipub++)
  {
    NDDS::DCPS::RepoIdConverter ipub_converter(*ipub);
  ...
  for (TAO_DDS_RepoId_Set::const_iterator isub = ignoredSubscriptions_.begin();
       ...
       isub++)
  {
    NDDS::DCPS::RepoIdConverter isub_converter(*isub);
