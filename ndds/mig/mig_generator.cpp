...
DCPS_IR_Domain::DCPS_IR_Domain(DDS::DomainId_t id, NDDS::DCPS::RepoIdGenerator& generator)
  : id_(id),
...
DCPS_IR_Participant_rch DCPS_IR_Domain::participant_rch(const NDDS::DCPS::RepoId& id) const
{
...
DCPS_IR_Participant* DCPS_IR_Domain::participant(const NDDS::DCPS::RepoId& id) const
{
...
{
  NDDS::DCPS::RepoId participantId = participant->get_id();
...
int DCPS_IR_Domain::remove_participant(const NDDS::DCPS::RepoId& participantId,
...
NDDS::DCPS::TopicStatus DCPS_IR_Domain::add_topic(NDDS::DCPS::RepoId_out topicId,
                                                     const char * topicName,
                                                     const char * dataTypeName,
                                                     const DDS::TopicQos & qos,
                                                     DCPS_IR_Participant* participantPtr)
{
  topicId = NDDS::DCPS::GUID_UNKNOWN;

  bool isBIT = NDDS::DCPS::topicIsBIT(topicName, dataTypeName);
  NDDS::DCPS::RepoId new_topic_id = participantPtr->get_next_topic_id(isBIT);
  NDDS::DCPS::TopicStatus status = add_topic_i(new_topic_id, topicName
                                                  , dataTypeName
                                                  , qos, participantPtr, isBIT);

  if (status == NDDS::DCPS::CREATED) {
...
NDDS::DCPS::TopicStatus
DCPS_IR_Domain::force_add_topic(const NDDS::DCPS::RepoId& topicId,
                                const char* topicName,
                                const char* dataTypeName,
                                const DDS::TopicQos & qos,
                                DCPS_IR_Participant* participantPtr)
{
  NDDS::DCPS::RepoId topic_id = topicId;
  bool isBIT = NDDS::DCPS::topicIsBIT(topicName, dataTypeName);
  NDDS::DCPS::TopicStatus status = add_topic_i(topic_id, topicName
                                                  , dataTypeName
                                                  , qos, participantPtr, isBIT);
...
NDDS::DCPS::TopicStatus DCPS_IR_Domain::add_topic_i(NDDS::DCPS::RepoId& topicId,
                                                       const char * topicName,
                                                       const char * dataTypeName,
                                                       const DDS::TopicQos & qos,
                                                       DCPS_IR_Participant* participantPtr,
                                                       bool isBIT)
{
  ...
  if (1 == descriptionLookup) {
    topicId = NDDS::DCPS::GUID_UNKNOWN;
    return NDDS::DCPS::CONFLICTING_TYPENAME;
    ...
    if (0 != descriptionAddition) {
      ...
      if (2 == descriptionAddition) {
        ...
        return NDDS::DCPS::NOT_FOUND;
      }
    }
  }

  NDDS::DCPS::unique_ptr<DCPS_IR_Topic> topic( new
                 DCPS_IR_Topic(
                   topicId,
                   qos,
                   this,
  ...
  NDDS::DCPS::TopicStatus topicStatus = NDDS::DCPS::NOT_FOUND;
  ...
  case 0: {
    ...
    case 1:
      ...
      topicStatus = NDDS::DCPS::NOT_FOUND;
      topicId = NDDS::DCPS::GUID_UNKNOWN;
...
DCPS_IR_Domain::find_topic(const char* topicName, DCPS_IR_Topic*& topic)
{
  ...
  if (which != this->topicDescriptions_.end()) {
    ...
    if (NDDS::DCPS::DCPS_debug_level > 0) {
      NDDS::DCPS::RepoId topicId = topic->get_id();
      NDDS::DCPS::RepoIdConverter converter(topicId);
      ACE_DEBUG((LM_DEBUG,
...
DCPS_IR_Topic*
DCPS_IR_Domain::find_topic(const NDDS::DCPS::RepoId& id)
{
...
NDDS::DCPS::TopicStatus DCPS_IR_Domain::remove_topic(DCPS_IR_Participant* part,
                                                        DCPS_IR_Topic*& topic)
{
  ...
  if (part->remove_topic_reference(topic->get_id(), topic) != 0) {
    NDDS::DCPS::RepoIdConverter part_converter(part->get_id());
    NDDS::DCPS::RepoIdConverter topic_converter(topic->get_id());
    ACE_ERROR((LM_ERROR,
...
int DCPS_IR_Domain::init_built_in_topics(bool federated, bool persistent)
{
  ...
  try {
    ...
    bitParticipantListener_ = new NDDS_DCPS_DomainParticipantListener_i;
  ...
  try {
    ...
    if (DDS::RETCODE_OK !=
        participantTypeSupport->register_type(bitParticipant_.in(),
                                              NDDS::DCPS::BUILT_IN_PARTICIPANT_TOPIC_TYPE)) {
      ACE_ERROR((LM_ERROR,
    ...
    bitParticipantTopic_ =
      bitParticipant_->create_topic(NDDS::DCPS::BUILT_IN_PARTICIPANT_TOPIC,
                                    NDDS::DCPS::BUILT_IN_PARTICIPANT_TOPIC_TYPE,
                                    topic_qos,
    ...
    if (CORBA::is_nil(bitParticipantTopic_.in())) {
      ACE_ERROR_RETURN((LM_ERROR,
                        ...
                        ACE_TEXT("DCPS_IR_Domain::init_built_in_topics.\n"),
                        NDDS::DCPS::BUILT_IN_PARTICIPANT_TOPIC),
                       1);
    }
    ...
    if (DDS::RETCODE_OK !=
        topicTypeSupport->register_type(bitParticipant_.in(),
                                        NDDS::DCPS::BUILT_IN_TOPIC_TOPIC_TYPE)) {
      ACE_ERROR((LM_ERROR,
    ...
    bitTopicTopic_ =
      bitParticipant_->create_topic(NDDS::DCPS::BUILT_IN_TOPIC_TOPIC,
                                    NDDS::DCPS::BUILT_IN_TOPIC_TOPIC_TYPE,
                                    topic_qos,
                                    DDS::TopicListener::_nil(),
                                    NDDS::DCPS::DEFAULT_STATUS_MASK);
    ...
    if (CORBA::is_nil(bitTopicTopic_.in())) {
      ACE_ERROR_RETURN((LM_ERROR,
                        ...
                        ACE_TEXT("Nil %C Topic from ")
                        ACE_TEXT("DCPS_IR_Domain::init_built_in_topics.\n"),
                        NDDS::DCPS::BUILT_IN_TOPIC_TOPIC),
                       1);
    }
    ...
    if (DDS::RETCODE_OK !=
        subscriptionTypeSupport->register_type(bitParticipant_.in(),
                                               NDDS::DCPS::BUILT_IN_SUBSCRIPTION_TOPIC_TYPE)) {
      ACE_ERROR((LM_ERROR,
    ...
    bitSubscriptionTopic_ =
      bitParticipant_->create_topic(NDDS::DCPS::BUILT_IN_SUBSCRIPTION_TOPIC,
                                    NDDS::DCPS::BUILT_IN_SUBSCRIPTION_TOPIC_TYPE,
                                    topic_qos,
                                    DDS::TopicListener::_nil(),
                                    NDDS::DCPS::DEFAULT_STATUS_MASK);
    ...
    if (CORBA::is_nil(bitSubscriptionTopic_.in())) {
      ACE_ERROR_RETURN((LM_ERROR,
                        ...
                        ACE_TEXT("Nil %C Topic from ")
                        ACE_TEXT("DCPS_IR_Domain::init_built_in_topics.\n"),
                        NDDS::DCPS::BUILT_IN_SUBSCRIPTION_TOPIC),
                       1);
    }
    ...
    if (DDS::RETCODE_OK !=
        publicationTypeSupport->register_type(bitParticipant_.in(),
                                              NDDS::DCPS::BUILT_IN_PUBLICATION_TOPIC_TYPE)) {
      ACE_ERROR((LM_ERROR,
    ...
    bitPublicationTopic_ =
      bitParticipant_->create_topic(NDDS::DCPS::BUILT_IN_PUBLICATION_TOPIC,
                                    NDDS::DCPS::BUILT_IN_PUBLICATION_TOPIC_TYPE,
                                    topic_qos,
                                    DDS::TopicListener::_nil(),
                                    NDDS::DCPS::DEFAULT_STATUS_MASK);
    ...
    if (CORBA::is_nil(bitPublicationTopic_.in())) {
      ACE_ERROR_RETURN((LM_ERROR,
                        ...
                        ACE_TEXT("Nil %C Topic from ")
                        ACE_TEXT("DCPS_IR_Domain::init_built_in_topics.\n"),
                        NDDS::DCPS::BUILT_IN_PUBLICATION_TOPIC),
                       1);
    }
...
int DCPS_IR_Domain::init_built_in_topics_transport(bool persistent)
{
  ...
  try {
    ...
    transportConfig_ =
      NDDS::DCPS::TransportRegistry::instance()->get_config(config_name);
    ...
    NDDS::DCPS::TransportRegistry::instance()->bind_config(transportConfig_,
                                                              bitPublisher_.in());
...
int DCPS_IR_Domain::cleanup_built_in_topics()
{
  ...
  if (useBIT_ && bitParticipant_) {
    using NDDS::DCPS::retcode_to_string;
...
int DCPS_IR_Domain::add_topic_description(NDDS::DCPS::unique_ptr<DCPS_IR_Topic_Description> desc)
{
  using NDDS::DCPS::move;
...
void DCPS_IR_Domain::remove_dead_participants()
{
  if (0 < deadParticipants_.size()) {
    ...
    while (iter != deadParticipants_.end()) {
      ...
      NDDS::DCPS::RepoIdConverter converter(dead->get_id());
      ACE_ERROR((LM_ERROR,
    ...
    try {
      ...
      if (NDDS::DCPS::DCPS_debug_level > 0) {
        ACE_DEBUG((LM_DEBUG,
                   ...
                   NDDS_STRING(NDDS::DCPS::GuidConverter(NDDS::DCPS::bit_key_to_repo_id(data.key))).c_str(), handle));
      }
...
void DCPS_IR_Domain::publish_topic_bit(DCPS_IR_Topic* topic)
{
  ...
  if (useBIT_) {
    ...
    if (!NDDS::DCPS::topicIsBIT(name, type)) {
      try {
...
void DCPS_IR_Domain::publish_publication_bit(DCPS_IR_Publication* publication)
{
  ...
  if (useBIT_) {
    ...
    bool isNotBIT =
      ACE_OS::strcmp(dataTypeName, NDDS::DCPS::BUILT_IN_PARTICIPANT_TOPIC_TYPE) &&
      ACE_OS::strcmp(dataTypeName, NDDS::DCPS::BUILT_IN_TOPIC_TOPIC_TYPE) &&
      ACE_OS::strcmp(dataTypeName, NDDS::DCPS::BUILT_IN_SUBSCRIPTION_TOPIC_TYPE) &&
      ACE_OS::strcmp(dataTypeName, NDDS::DCPS::BUILT_IN_PUBLICATION_TOPIC_TYPE);
    ...
    if (!participant->isBitPublisher()) {
      try {
        ...
        if (DDS::RETCODE_OK != retGetKey) {
          NDDS::DCPS::RepoIdConverter converter(participant->get_id());
          ACE_ERROR((LM_ERROR,
...
void DCPS_IR_Domain::remove_topic_id_mapping(const NDDS::DCPS::RepoId& topicId)
{
...
std::string DCPS_IR_Domain::dump_to_string(const std::string& prefix, int depth) const
{
  ...
  for (DCPS_IR_Participant_Set::const_iterator dp = deadParticipants_.begin();
       ...
       dp++)
  {
    NDDS::DCPS::RepoIdConverter sub_converter((*dp)->get_id());
