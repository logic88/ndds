...
DCPS_IR_Subscription::DCPS_IR_Subscription(const NDDS::DCPS::RepoId& id,
...
{
  reader_ =  NDDS::DCPS::DataReaderRemote::_duplicate(reader);
                                                     ...
                                                     bool active)
{
  ...
  case 0: {
    ...
    NDDS::DCPS::WriterAssociation association;
    ...
    if (participant_->is_alive() && this->participant_->isOwner()) {
      try {
        if (NDDS::DCPS::DCPS_debug_level > 0) {
          NDDS::DCPS::RepoIdConverter sub_converter(id_);
          NDDS::DCPS::RepoIdConverter pub_converter(pub->get_id());
          ACE_DEBUG((LM_DEBUG,
...
void DCPS_IR_Subscription::disassociate_participant(NDDS::DCPS::RepoId id,
                                                    bool reassociate)
{
  DCPS_IR_Publication* pub = 0;
  size_t numAssociations = associations_.size();
  ...
  if (0 < numAssociations) {
    NDDS::DCPS::WriterIdSeq idSeq(static_cast<CORBA::ULong>(numAssociations));
    ...
    while (iter != end) {
      ...
      if (NDDS::DCPS::DCPS_debug_level > 0) {
        NDDS::DCPS::RepoIdConverter sub_converter(id_);
        NDDS::DCPS::RepoIdConverter pub_converter(pub->get_id());
        NDDS::DCPS::RepoIdConverter sub_part_converter(id);
        NDDS::DCPS::RepoIdConverter pub_part_converter(pub->get_participant_id());
        ACE_DEBUG((LM_DEBUG,
...
void DCPS_IR_Subscription::disassociate_topic(NDDS::DCPS::RepoId id)
{
  DCPS_IR_Publication* pub = 0;
  size_t numAssociations = associations_.size();
  CORBA::Boolean dontSend = 0;
  ...
  if (0 < numAssociations) {
    ...
    while (iter != end) {
      ...
      if (NDDS::DCPS::DCPS_debug_level > 0) {
        NDDS::DCPS::RepoIdConverter sub_converter(id_);
        NDDS::DCPS::RepoIdConverter pub_converter(pub->get_id());
        NDDS::DCPS::RepoIdConverter sub_topic_converter(id);
        NDDS::DCPS::RepoIdConverter pub_topic_converter(pub->get_topic_id());
        ACE_DEBUG((LM_DEBUG,
...
void DCPS_IR_Subscription::disassociate_publication(NDDS::DCPS::RepoId id,
                                                    bool reassociate)
{
  DCPS_IR_Publication* pub = 0;
  size_t numAssociations = associations_.size();
  ...
  if (0 < numAssociations) {
    ...
    while (iter != end) {
      ...
      if (NDDS::DCPS::DCPS_debug_level > 0) {
        NDDS::DCPS::RepoIdConverter sub_converter(id_);
        NDDS::DCPS::RepoIdConverter pub_converter(pub->get_id());
        NDDS::DCPS::RepoIdConverter sub_pub_converter(id);
        ACE_DEBUG((LM_DEBUG,
...
CORBA::Boolean DCPS_IR_Subscription::is_publication_ignored(NDDS::DCPS::RepoId partId,
...
NDDS::DCPS::TransportLocatorSeq DCPS_IR_Subscription::get_transportLocatorSeq() const
{
  return info_;
}

NDDS::DCPS::IncompatibleQosStatus* DCPS_IR_Subscription::get_incompatibleQosStatus()
{
...
DCPS_IR_Subscription::set_qos(const DDS::DataReaderQos& qos)
{
  if (false == (qos == this->qos_)) {
    ...
    bool check =
      NDDS::DCPS::should_check_association_upon_change(qos, this->qos_);
...
DCPS_IR_Subscription::set_qos(const DDS::SubscriberQos& qos)
{
  if (false == (qos == this->subscriberQos_)) {
    // Check if we should check while we have both values.
    bool check = NDDS::DCPS::should_check_association_upon_change(qos, this->subscriberQos_);
                                   ...
                                   Update::SpecificQos& specificQos)
{
  ...
  if (u_sub_qos) {
    if (NDDS::DCPS::should_check_association_upon_change(subscriberQos_, subscriberQos)) {
      need_evaluate = true;
    }
...
DCPS_IR_Subscription::reevaluate_defunct_associations()
{
  ...
  while (it != this->defunct_.end()) {
    ...
    if (reevaluate_association(publication)) {
      ...
      NDDS::DCPS::RepoIdConverter sub_converter(id_);
      NDDS::DCPS::RepoIdConverter pub_converter(publication->get_id());
      ACE_ERROR((LM_ERROR,
...
DCPS_IR_Subscription::reevaluate_association(DCPS_IR_Publication* publication)
{
  ...
  if (status == 0) {
    ...
    if (!NDDS::DCPS::compatibleQOS(publication->get_incompatibleQosStatus(),
...
NDDS::DCPS::RepoId DCPS_IR_Subscription::get_id()
{
  return id_;
}

NDDS::DCPS::RepoId DCPS_IR_Subscription::get_topic_id()
{
  return topic_->get_id();
}

NDDS::DCPS::RepoId DCPS_IR_Subscription::get_participant_id()
{
...
NDDS::DCPS::DataReaderRemote_ptr
DCPS_IR_Subscription::reader()
{
  return NDDS::DCPS::DataReaderRemote::_duplicate(this->reader_.in());
}
