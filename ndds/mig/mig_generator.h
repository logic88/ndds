...
typedef std::map<std::string,
                 NDDS::DCPS::container_supported_unique_ptr<DCPS_IR_Topic_Description> > DCPS_IR_Topic_Description_Set;

class DCPS_IR_Participant;
typedef NDDS::DCPS::RcHandle<DCPS_IR_Participant> DCPS_IR_Participant_rch;
typedef std::set<DCPS_IR_Participant_rch > DCPS_IR_Participant_Set;

typedef std::map<NDDS::DCPS::RepoId,
                 DCPS_IR_Participant_rch ,
                 NDDS::DCPS::GUID_tKeyLessThan> DCPS_IR_Participant_Map;
 ...
 * system's domain.
 */
class NDDS_InfoRepoLib_Export DCPS_IR_Domain
: public NDDS::DCPS::RcObject {
public:
  DCPS_IR_Domain(DDS::DomainId_t id, NDDS::DCPS::RepoIdGenerator& generator);
  ...
  int remove_participant(const NDDS::DCPS::RepoId& particpantId,
                         CORBA::Boolean    notify_lost);

  DCPS_IR_Participant_rch participant_rch(const NDDS::DCPS::RepoId& id) const;

  /// Find the participant with the id.
  DCPS_IR_Participant* participant(const NDDS::DCPS::RepoId& id) const;

  ///@{
  /// Add a topic to the domain
  /// Returns NDDS::DCPS::CREATED if successful
  NDDS::DCPS::TopicStatus add_topic(NDDS::DCPS::RepoId_out topicId,
                                       const char * topicName,
                                       const char * dataTypeName,
                                       const DDS::TopicQos & qos,
                                       DCPS_IR_Participant* participantPtr);

  NDDS::DCPS::TopicStatus force_add_topic(const NDDS::DCPS::RepoId& topicId,
                                             const char* topicName,
                                             const char* dataTypeName,
                                             const DDS::TopicQos & qos,
                                             DCPS_IR_Participant* participantPtr);
  ...
  /// Returns NDDS::DCPS::FOUND if exists and topic is changed, -1 otherwise
  NDDS::DCPS::TopicStatus find_topic(const char * topicName,
                                        DCPS_IR_Topic*& topic);

  /// Find a topic object reference using the topic Id value.
  DCPS_IR_Topic* find_topic(const NDDS::DCPS::RepoId& id);

  /// Remove the topic
  /// The topic has been deleted if returns successful
  /// Returns NDDS::DCPS::REMOVED if successful
  NDDS::DCPS::TopicStatus remove_topic(DCPS_IR_Participant* part,
                                          DCPS_IR_Topic*& topic);

  /// Remove the topic from the id to topic map.
  /// This method should only be called by the DCPS_IR_Topic
  /// when deleting the topic.
  void remove_topic_id_mapping(const NDDS::DCPS::RepoId& topicId);
  ...
  NDDS::DCPS::RepoId get_next_participant_id();
...
private:
  NDDS::DCPS::TopicStatus add_topic_i(NDDS::DCPS::RepoId& topicId,
                                         const char * topicName,
                                         const char * dataTypeName,
                                         const DDS::TopicQos & qos,
                                         DCPS_IR_Participant* participantPtr,
  ...
  int add_topic_description(NDDS::DCPS::unique_ptr<DCPS_IR_Topic_Description> desc);
  ...
  NDDS::DCPS::RepoIdGenerator& participantIdGenerator_;
  ...
  typedef std::map<NDDS::DCPS::RepoId,
                   NDDS::DCPS::container_supported_unique_ptr<DCPS_IR_Topic>,
                   NDDS::DCPS::GUID_tKeyLessThan> IdToTopicMap;
...
#if !defined (DDS_HAS_MINIMUM_BIT)
  NDDS::DCPS::TransportConfig_rch               transportConfig_;
