...
using NDDS::FaceTSS::config::ConnectionSettings;
using NDDS::FaceTSS::config::TopicSettings;
using NDDS::FaceTSS::config::QosSettings;

namespace {
  NDDS::FaceTSS::config::Parser parser;
                       ...
                       RETURN_CODE_TYPE& return_code)
{
  try {
    ...
    const SYSTEM_TIME_TYPE refresh_period =
      ...
      NDDS::FaceTSS::convertDuration(qos.datawriter_qos().lifespan.duration) : 0;
  ...
  try {
    ...
    if (connection_id != 0 && entities.connections_.count(connection_id)) {
      ...
      if (connection_name[0]) {
        ...
        NDDS_STRING conn_name = entities.connections_[connection_id].connection_name;
                    ...
                    /*out*/   FACE::RETURN_CODE_TYPE& return_code)
{
  try {
    ...
    if (transaction_id == readers[connection_id]->last_msg_tid) {
      ...
      return_code = NDDS::FaceTSS::update_status(connection_id, DDS::RETCODE_OK);
    } else {
      return_code = NDDS::FaceTSS::update_status(connection_id, DDS::RETCODE_BAD_PARAMETER);
    }
  } catch (const CORBA::BAD_PARAM&) {
    if (NDDS::DCPS::DCPS_debug_level) {
...
namespace {
  ...
  RETURN_CODE_TYPE create_opendds_entities(CONNECTION_ID_TYPE connectionId,
                                           ...
                                           const char* transport)
  {
#ifdef DEBUG_NDDS_FACETSS
    NDDS::DCPS::set_DCPS_debug_level(8);
    NDDS::DCPS::Transport_debug_level = 5;
    ...
    using NDDS::DCPS::Data_Types_Register;
    NDDS::DCPS::TypeSupport_var ts =
    ...
    if (dir == SOURCE) {
      ...
      if (transport && transport[0]) {
        NDDS::DCPS::TransportConfig_rch config = TheTransportRegistry->get_config(transport);
        if (config.is_nil()) return INVALID_PARAM;
        try {
          TheTransportRegistry->bind_config(config, pub);
        } catch (const NDDS::DCPS::Transport::Exception&) {
...
create_message_instance_guid(const NDDS::DCPS::RepoId& pub, const CORBA::LongLong& orig_seq)
{
  NDDS::DCPS::GuidConverter writer(pub);
                              ...
                              FACE::RETURN_CODE_TYPE& return_code)
{
  ...
  NDDS::DCPS::DomainParticipantImpl* dpi = dynamic_cast<NDDS::DCPS::DomainParticipantImpl*>(temp_dp.in());
  if (!dpi) {
  ...
  const NDDS::DCPS::RepoId pub = dpi->get_repoid(sinfo.publication_handle);
  ...
  const NDDS::DCPS::SystemTimePoint now = NDDS::DCPS::SystemTimePoint::now();
  ...
  if (NDDS::DCPS::DCPS_debug_level > 8) {
  ...
  const NDDS::DCPS::RepoId sub = dpi->get_id();
  ...
  if (std::memcmp(pub.guidPrefix, sub.guidPrefix, sizeof(sub.guidPrefix)) == 0) {
    ...
    ::DDS::DataReader_var reader
     = bit_subscriber->lookup_datareader (NDDS::DCPS::BUILT_IN_PUBLICATION_TOPIC) ;
...
//                         pubdata[i].lifespan.duration.sec, pubdata[i].lifespan.duration.nanosec));
  ...
  if (lifespan.sec != DDS::DURATION_INFINITE_SEC &&
    ...
    const NDDS::DCPS::SystemTimePoint expiration_time(tmp);
