#ifndef NDDS_FACE_FACETSS_H
#define NDDS_FACE_FACETSS_H
...
public:
  struct DDSAdapter : public NDDS::DCPS::PoolAllocationBase {
    DDSAdapter ()
  ...
  typedef NDDS_MAP(FACE::CONNECTION_ID_TYPE, FaceSender) ConnIdToSenderMap;
  typedef NDDS_MAP(FACE::CONNECTION_ID_TYPE, FaceReceiver*) ConnIdToReceiverMap;

  NDDS_FACE_Export static Entities* instance();
  ConnIdToSenderMap senders_;
  ConnIdToReceiverMap receivers_;

  struct ConnectionInfo {
    NDDS_STRING connection_name;
    FACE::TRANSPORT_CONNECTION_STATUS_TYPE connection_status;
    FACE::MESSAGE_TYPE_GUID platform_view_guid;
  };
  NDDS_MAP(FACE::CONNECTION_ID_TYPE, ConnectionInfo ) connections_;
};

NDDS_FACE_Export DDS::Duration_t convertTimeout(FACE::TIMEOUT_TYPE timeout);
NDDS_FACE_Export FACE::SYSTEM_TIME_TYPE convertDuration(const DDS::Duration_t& duration);
NDDS_FACE_Export FACE::SYSTEM_TIME_TYPE convertTime(const DDS::Time_t& timestamp);
NDDS_FACE_Export void populate_header_received(const FACE::CONNECTION_ID_TYPE& connection_id,
                                                  const DDS::DomainParticipant_var part,
                                                  const DDS::SampleInfo& sinfo,
                                                  FACE::RETURN_CODE_TYPE& return_code);

NDDS_FACE_Export FACE::MESSAGE_INSTANCE_GUID create_message_instance_guid(const NDDS::DCPS::RepoId& pub,
                                                                             const CORBA::LongLong& seq);

NDDS_FACE_Export FACE::RETURN_CODE_TYPE update_status(FACE::CONNECTION_ID_TYPE connection_id,
...
private:
  ...
  NDDS_VECTOR(Callback) callbacks_;
