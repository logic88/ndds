        ...
        if (next == it->transport_seq_.first.getValue()) {
          ...
          if (!it->rec_ds_.sample_) {
            ...
            if (!fr.rec_ds_.sample_) {
              ACE_ERROR((LM_ERROR,
                         ACE_TEXT("(%P|%t) ERROR: ")
                         ACE_TEXT("NDDS::DCPS::TransportReassembly::insert, ")
                              ...
                              CORBA::ULong& numBits) const
{
  ...
  const NDDS_LIST(FragRange)& flist = iter->second.range_list_;
  ...
  typedef NDDS_LIST(FragRange)::const_iterator list_iterator;
                                  ...
                                  ACE_UINT32 total_frags)
{
  if (Transport_debug_level > 5) {
    ...
    ACE_DEBUG((LM_DEBUG, "(%P|%t) DBG:   TransportReassembly::reassemble_i "
      ...
      data.header_.sequence_.getValue(), NDDS_STRING(conv).c_str()));
  }
...
TransportReassembly::data_unavailable(const SequenceRange& dropped)
{
  ...
  typedef NDDS_LIST(FragRange)::iterator list_iterator;
