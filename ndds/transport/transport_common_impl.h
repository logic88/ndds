...
#ifndef NDDS_DCPS_TRANSPORT_FRAMEWORK_TRANSPORTREASSEMBLY_H
#define NDDS_DCPS_TRANSPORT_FRAMEWORK_TRANSPORTREASSEMBLY_H
...
#pragma pack(pop)
  ...
  NDDS_OOAT_CUSTOM_HASH(FragKey, NDDS_Dcps_Export, FragKeyHash);
#endif

class NDDS_Dcps_Export TransportReassembly : public RcObject {
public:
...
private:
  ...
  typedef NDDS_LIST(FragRange) FragRangeList;
  typedef NDDS_MAP(SequenceNumber::Value, FragRangeList::iterator) FragRangeIterMap;
...
#ifdef ACE_HAS_CPP11
  typedef NDDS_UNORDERED_MAP_CHASH(FragKey, FragInfo, FragKeyHash) FragInfoMap;
#else
  typedef NDDS_MAP(FragKey, FragInfo) FragInfoMap;
#endif
  FragInfoMap fragments_;

  typedef std::pair<MonotonicTimePoint, FragKey> ElementType;
  typedef NDDS_LIST(ElementType) ExpirationQueue;
  ExpirationQueue expiration_queue_;

  typedef NDDS_MAP_CMP(PublicationId, DisjointSequence, GUID_tKeyLessThan) CompletedMap;
