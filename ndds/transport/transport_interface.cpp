...
namespace {
  /// Used for sorting
  bool predicate(const NDDS::DCPS::TransportInst_rch& lhs,
                 const NDDS::DCPS::TransportInst_rch& rhs)
  {
    return lhs->name() < rhs->name();
  }

  // transport type to try loading if none are loaded when DCPS attempts to use
#ifdef NDDS_SAFETY_PROFILE
...
const char TransportRegistry::DEFAULT_CONFIG_NAME[] = "_NDDS_DEFAULT_CONFIG";
const char TransportRegistry::DEFAULT_INST_PREFIX[] = "_NDDS_";

// transport template customizations
const NDDS_STRING TransportRegistry::CUSTOM_ADD_DOMAIN_TO_IP = "add_domain_id_to_ip_addr";
const NDDS_STRING TransportRegistry::CUSTOM_ADD_DOMAIN_TO_PORT = "add_domain_id_to_port";
  ...
  , load_complete_(lock_)
{
  ...
  lib_directive_map_["tcp"]       = "dynamic NDDS_Tcp Service_Object * NDDS_Tcp:_make_TcpLoader()";
  lib_directive_map_["udp"]       = "dynamic NDDS_Udp Service_Object * NDDS_Udp:_make_UdpLoader()";
  lib_directive_map_["multicast"] = "dynamic NDDS_Multicast Service_Object * NDDS_Multicast:_make_MulticastLoader()";
  lib_directive_map_["rtps_udp"]  = "dynamic NDDS_Rtps_Udp Service_Object * NDDS_Rtps_Udp:_make_RtpsUdpLoader()";
  lib_directive_map_["shmem"]     = "dynamic NDDS_Shmem Service_Object * NDDS_Shmem:_make_ShmemLoader()";

  // load_transport_lib() is used for discovery as well:
  lib_directive_map_["rtps_discovery"] = lib_directive_map_["rtps_udp"];
  lib_directive_map_["repository"] = "dynamic NDDS_InfoRepoDiscovery Service_Object * NDDS_InfoRepoDiscovery:_make_IRDiscoveryLoader()";
}

int
TransportRegistry::load_transport_configuration(const NDDS_STRING& file_name,
                                                ACE_Configuration_Heap& cf)
{
  ...
  typedef std::pair<TransportConfig_rch, NDDS_VECTOR(NDDS_STRING) > ConfigInfo;
  NDDS_VECTOR(ConfigInfo) configInfoVec;

  // Record the transport instances created, so we can place them
  // in the implicit transport configuration for this file.
  NDDS_LIST(TransportInst_rch) instances;
  ...
  for (int index = 0;
    ...
    if (ACE_OS::strcmp(sect_name.c_str(), TRANSPORT_SECTION_NAME) == 0 ||
      ...
      if (cf.open_section(root, sect_name.c_str(), false, sect) != 0) {
        ...
        for (KeyList::const_iterator it = keys.begin(); it != keys.end(); ++it) {
          ...
          if (pullValues(cf, it->second, values) != 0) {
            ...
            // object.
            const NDDS_STRING tid_str = transport_id.c_str() ? ACE_TEXT_ALWAYS_CHAR(transport_id.c_str()) : "";
      ...
      if (cf.open_section(root, sect_name.c_str(), false, sect) != 0) {
        ...
        for (KeyList::const_iterator it = keys.begin(); it != keys.end(); ++it) {
          NDDS_STRING config_id = it->first;
          ...
          for (ValueMap::const_iterator it = values.begin(); it != values.end(); ++it) {
            NDDS_STRING name = it->first;
            NDDS_STRING value = it->second;
            if (name == "transports") {
              char delim = ',';
              size_t pos = 0;
              NDDS_STRING token;
              while ((pos = value.find(delim)) != NDDS_STRING::npos) {
              ...
              for (NDDS_VECTOR(TransportTemplate)::iterator it = transport_templates_.begin(); it != transport_templates_.end(); ++it) {
              ...
              for (NDDS_VECTOR(TransportEntry)::iterator it = transports_.begin(); it != transports_.end(); ++it) {
  ...
  for (unsigned int i = 0; i < configInfoVec.size(); ++i) {
    ...
    NDDS_VECTOR(NDDS_STRING)& insts = configInfoVec[i].second;
  ...
  if (!instances.empty()) {
    ...
    for (NDDS_LIST(TransportInst_rch)::const_iterator it = instances.begin();
         it != instances.end(); ++it) {
    ...
    for (KeyList::const_iterator it = keys.begin(); it != keys.end(); ++it) {
      ...
      for (ValueMap::const_iterator it = values.begin(); it != values.end(); ++it) {
        ...
        if (name == "instantiation_rule") {
          ...
          if (DCPS_debug_level > 0) {
            NDDS_STRING flag = element.instantiate_per_participant ? "true" : "false";
            ACE_DEBUG((LM_DEBUG,
                       ACE_TEXT("(%P|%t) TransportRegistry::load_transport_templates(): ")
...
void
TransportRegistry::load_transport_lib(const NDDS_STRING& transport_type,
                                      bool wait)
{
...
TransportType_rch
TransportRegistry::load_transport_lib_i(const NDDS_STRING& transport_type,
                                        bool wait)
{
...
TransportInst_rch
TransportRegistry::create_inst(const NDDS_STRING& name,
...
TransportInst_rch
TransportRegistry::get_inst(const NDDS_STRING& name) const
{
  GuardType guard(lock_);
...
TransportConfig_rch
TransportRegistry::create_config(const NDDS_STRING& name)
{
  GuardType guard(lock_);
...
TransportConfig_rch
TransportRegistry::get_config(const NDDS_STRING& name) const
{
  GuardType guard(lock_);
                               ...
                               DDS::Entity_ptr entity)
{
  ...
  if (TheServiceParticipant->belongs_to_domain_range(domain_id)) {
    bool found = false;
    for (NDDS_VECTOR(TransportTemplate)::const_iterator i = transport_templates_.begin(); i != transport_templates_.end(); ++i) {
    ...
    if (found)
    {
      ...
      if (ret == 0) {
        ...
        NDDS_STRING transport_inst_name = GuidConverter(guid).uniqueParticipantId();
        NDDS_STRING transport_config_name;
...
void
TransportRegistry::remove_transport_template_instance(const NDDS_STRING& config_name)
{
  ...
  const NDDS_STRING inst_name = i->second;
...
TransportRegistry::register_type(const TransportType_rch& type)
{
  ...
  const NDDS_STRING name = type->name();
...
bool
TransportRegistry::create_new_transport_instance_for_participant(DDS::DomainId_t id, NDDS_STRING& transport_config_name, NDDS_STRING& transport_instance_name)
{
  ...
  NDDS_STRING inst_name = cfg->instances_[0]->name() + "_" + transport_instance_name;
  NDDS_STRING config_name = cfg->name() + "_" + transport_instance_name;

  // assign new config and inst names
  transport_config_name = config_name;
  transport_instance_name = inst_name;

  NDDS::DCPS::TransportConfig_rch config = create_config(config_name);
  NDDS::DCPS::TransportInst_rch inst = create_inst(inst_name, "rtps_udp");
...
void
TransportRegistry::update_config_template_instance_info(const NDDS_STRING& config_name, const NDDS_STRING& inst_name)
{
...
{
  NDDS::DCPS::Discovery::RepoKey configured_name = "transport_template_instance_";
...
{
  NDDS::DCPS::Discovery::RepoKey configured_name = "templ_config_";
  configured_name += to_dds_string(id);
  return configured_name;
}
...
{
  NDDS_STRING transport_inst_name = get_transport_template_instance_name(domain);
  NDDS_STRING config_inst_name = get_config_instance_name(domain);
...
bool TransportRegistry::process_customizations(const DDS::DomainId_t id, const TransportTemplate& tr_inst, ValueMap& customs)
{
  for (ValueMap::const_iterator it = tr_inst.transport_info.begin();
    ...
    if (idx != tr_inst.customizations.end()) {
      NDDS_STRING addr = it->second;
      NDDS_STRING custom = idx->second;
      ...
      if (add_to_ip) {
        ...
        if (pos != NDDS_STRING::npos) {
          NDDS_STRING custom = addr.substr(pos + 1);

          NDDS_STRING port = "";
          NDDS_STRING last_octet = "";

          size_t cpos = custom.find(":");
          if (cpos != NDDS_STRING::npos) {
...
TransportRegistry::get_transport_info(const ACE_TString& config_name, TransportEntry& inst)
{
  bool ret = false;
  if (has_transports()) {
    for (NDDS_VECTOR(TransportEntry)::const_iterator i = transports_.begin(); i != transports_.end(); ++i) {
