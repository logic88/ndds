...
DCPS_IR_Publication::DCPS_IR_Publication(const NDDS::DCPS::RepoId& id,
...
{
  writer_ =  NDDS::DCPS::DataWriterRemote::_duplicate(writer);
                                                     ...
                                                     bool active)
{
  ...
  case 0: {
    ...
    NDDS::DCPS::ReaderAssociation association;
    ...
    if (participant_->is_alive() && this->participant_->isOwner()) {
      try {
        if (NDDS::DCPS::DCPS_debug_level > 0) {
          NDDS::DCPS::RepoIdConverter pub_converter(id_);
          NDDS::DCPS::RepoIdConverter sub_converter(sub->get_id());
          ACE_DEBUG((LM_DEBUG,
      ...
      try {
        if (NDDS::DCPS::DCPS_debug_level > 0) {
          ...
          ACE_DEBUG((LM_DEBUG,
                     ...
                     NDDS_STRING(pub_converter).c_str(),
                     NDDS_STRING(sub_converter).c_str()));
        }

        NDDS::DCPS::ReaderIdSeq idSeq(1);
        idSeq.length(1);
        idSeq[0] = sub->get_id();
...
void DCPS_IR_Publication::disassociate_participant(NDDS::DCPS::RepoId id,
                                                   bool reassociate)
{
  DCPS_IR_Subscription* sub = 0;
  size_t numAssociations = associations_.size();
  ...
  if (0 < numAssociations) {
    NDDS::DCPS::ReaderIdSeq idSeq(static_cast<CORBA::ULong>(numAssociations));
    ...
    while (iter != end) {
      ...
      if (NDDS::DCPS::DCPS_debug_level > 0) {
        NDDS::DCPS::RepoIdConverter pub_converter(id_);
        NDDS::DCPS::RepoIdConverter sub_converter(sub->get_id());
        NDDS::DCPS::RepoIdConverter pub_part_converter(id);
        NDDS::DCPS::RepoIdConverter sub_part_converter(sub->get_participant_id());
        ACE_DEBUG((LM_DEBUG,
...
void DCPS_IR_Publication::disassociate_topic(NDDS::DCPS::RepoId id)
{
  DCPS_IR_Subscription* sub = 0;
  size_t numAssociations = associations_.size();
  CORBA::Boolean send = 1;
  ...
  if (0 < numAssociations) {
    ...
    while (iter != end) {
      ...
      if (NDDS::DCPS::DCPS_debug_level > 0) {
        NDDS::DCPS::RepoIdConverter pub_converter(id_);
        NDDS::DCPS::RepoIdConverter sub_converter(sub->get_id());
        NDDS::DCPS::RepoIdConverter pub_topic_converter(id);
        NDDS::DCPS::RepoIdConverter sub_topic_converter(sub->get_topic_id());
        ACE_DEBUG((LM_DEBUG,
...
void DCPS_IR_Publication::disassociate_subscription(NDDS::DCPS::RepoId id,
                                                    bool reassociate)
{
  DCPS_IR_Subscription* sub = 0;
  size_t numAssociations = associations_.size();
  ...
  if (0 < numAssociations) {
    ...
    while (iter != end) {
      ...
      if (NDDS::DCPS::DCPS_debug_level > 0) {
        NDDS::DCPS::RepoIdConverter pub_converter(id_);
        NDDS::DCPS::RepoIdConverter sub_converter(sub->get_id());
        NDDS::DCPS::RepoIdConverter pub_sub_converter(id);
        ACE_DEBUG((LM_DEBUG,
...
CORBA::Boolean DCPS_IR_Publication::is_subscription_ignored(NDDS::DCPS::RepoId partId,
...
DCPS_IR_Publication::set_qos(const DDS::DataWriterQos& qos)
{
  if (false == (qos == this->qos_)) {
    ...
    bool check = NDDS::DCPS::should_check_association_upon_change(qos, this->qos_);
...
DCPS_IR_Publication::set_qos(const DDS::PublisherQos& qos)
{
  if (false == (qos == this->publisherQos_)) {
    // Check if we should check while we have both values.
    bool check = NDDS::DCPS::should_check_association_upon_change(qos, this->publisherQos_);
                                  ...
                                  Update::SpecificQos& specificQos)
{
  ...
  if (u_dw_qos) {
    if (NDDS::DCPS::should_check_association_upon_change(qos_, qos)) {
      need_evaluate = true;
    }
  ...
  if (u_pub_qos) {
    if (NDDS::DCPS::should_check_association_upon_change(publisherQos_, publisherQos)) {
      need_evaluate = true;
    }
...
NDDS::DCPS::TransportLocatorSeq DCPS_IR_Publication::get_transportLocatorSeq() const
{
  return info_;
}

NDDS::DCPS::IncompatibleQosStatus* DCPS_IR_Publication::get_incompatibleQosStatus()
{
  return &incompatibleQosStatus_;
}

NDDS::DCPS::RepoId DCPS_IR_Publication::get_id()
{
  return id_;
}

NDDS::DCPS::RepoId DCPS_IR_Publication::get_topic_id()
{
  return topic_->get_id();
}

NDDS::DCPS::RepoId DCPS_IR_Publication::get_participant_id()
{
...
NDDS::DCPS::DataWriterRemote_ptr
DCPS_IR_Publication::writer()
{
  return NDDS::DCPS::DataWriterRemote::_duplicate(this->writer_.in());
}
...
DCPS_IR_Publication::reevaluate_defunct_associations()
{
  ...
  while (it != this->defunct_.end()) {
    ...
    if (reevaluate_association(subscription)) {
      ...
      NDDS::DCPS::RepoIdConverter pub_converter(id_);
      NDDS::DCPS::RepoIdConverter sub_converter(subscription->get_id());
      ACE_ERROR((LM_ERROR,
...
DCPS_IR_Publication::reevaluate_association(DCPS_IR_Subscription* subscription)
{
  ...
  if (status == 0) {
    ...
    if (!NDDS::DCPS::compatibleQOS(this->get_incompatibleQosStatus(),
...
void
DCPS_IR_Publication::update_expr_params(NDDS::DCPS::RepoId readerId,
...
DCPS_IR_Publication::dump_to_string(const std::string& prefix, int depth) const
{
  ...
  for (DCPS_IR_Subscription_Set::const_iterator assoc = associations_.begin();
       ...
       assoc++)
  {
    NDDS::DCPS::RepoIdConverter assoc_converter((*assoc)->get_id());
  ...
  for (DCPS_IR_Subscription_Set::const_iterator def = defunct_.begin();
       ...
       def++)
  {
    NDDS::DCPS::RepoIdConverter def_converter((*def)->get_id());
