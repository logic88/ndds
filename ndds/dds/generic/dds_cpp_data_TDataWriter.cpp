...
{
  NDDS_ASSERT(!a_local.foundation.empty());
  NDDS_ASSERT(!a_remote.foundation.empty());
}
...
{
  NDDS_ASSERT(valid_pair.use_candidate);
...
void Checklist::succeeded(const ConnectivityCheck& cc)
{
  ...
  if (cp.use_candidate) {
    if (local_is_controlling_) {
      ...
      nominating_ = valid_list_.end();
      NDDS_ASSERT(frozen_.empty());
      NDDS_ASSERT(waiting_.empty());
    }
    ...
    NDDS_ASSERT(frozen_.empty());
    NDDS_ASSERT(waiting_.empty());
    NDDS_ASSERT(triggered_check_queue_.empty());
    NDDS_ASSERT(in_progress_.empty());
    NDDS_ASSERT(connectivity_checks_.empty());
  }
                                 ...
                                 const STUN::Message& a_message)
{
  ...
  NDDS_ASSERT(pos != connectivity_checks_.end());
