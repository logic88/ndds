...
namespace {
  u_short get_default_d0(u_short fallback)
  {
    ...
    const char* from_env = std::getenv("NDDS_RTPS_DEFAULT_D0");
    if (from_env) {
...
RtpsDiscovery::Config::discovery_config(ACE_Configuration_Heap& cf)
{
  ...
  if (cf.open_section(root, RTPS_SECTION_NAME, 0, rtps_sect) == 0) {
    ...
    for (DCPS::KeyList::const_iterator it = keys.begin();
         it != keys.end(); ++it) {
      const NDDS_STRING& rtps_name = it->first;

      RtpsDiscovery_rch discovery = NDDS::DCPS::make_rch<RtpsDiscovery>(rtps_name);
      ...
      for (DCPS::ValueMap::const_iterator it = values.begin();
           it != values.end(); ++it) {
        const NDDS_STRING& name = it->first;
        if (name == "ResendPeriod") {
          const NDDS_STRING& value = it->second;
          int resend;
          ...
          do {
            ...
            spdp_send_addrs.push_back(value.substr(i, (n == NDDS_STRING::npos) ? n : n - i));
            i = value.find(',', i);
          } while (i++ != NDDS_STRING::npos); // skip past comma if there is one
          ...
          // In milliseconds.
          const NDDS_STRING& string_value = it->second;
          int int_value;
  ...
  if (discoveryMap.find(Discovery::DEFAULT_RTPS) == discoveryMap.end()) {
    TheServiceParticipant->add_discovery(NDDS::DCPS::make_rch<RtpsDiscovery>(Discovery::DEFAULT_RTPS));
  }
...
{
  NDDS::DCPS::RepoId id = GUID_UNKNOWN;
  ACE_GUARD_RETURN(ACE_Thread_Mutex, g, lock_, id);
  const NDDS_STRING guid_interface = config_->guid_interface();
...
{
  DCPS::AddDomainStatus ads = {NDDS::DCPS::RepoId(), false /*federated*/};
  {
