...
public:
  ...
  ArgCopier(NDDS::Federator::Config* config);
...
private:
  ...
  NDDS::Federator::Config* config_;

  /// How to treat the next argument.
  Action action_;
};

ArgCopier::ArgCopier(NDDS::Federator::Config* config)
  : config_(config),
    action_(COPY)
{
...
ArgCopier::operator()(ACE_TCHAR* arg)
{
  ...
  if (::NDDS::Federator::Config::FEDERATOR_CONFIG_OPTION == arg) {
    // Configuration file option, filename value is next arg.
    this->action_ = FILENAME;
    return;

  } else if (::NDDS::Federator::Config::FEDERATOR_ID_OPTION == arg) {
    // Federation Id option, Id value is next arg.
    this->action_ = IDVALUE;
    return;

  } else if (::NDDS::Federator::Config::FEDERATE_WITH_OPTION == arg) {
