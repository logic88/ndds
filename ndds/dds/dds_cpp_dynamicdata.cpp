...
bool checkAndAssignParams(NDDS::DCPS::ContentFilterProperty_t& dest,
                          const NDDS::DCPS::ContentFilterProperty_t& src)
{
...
#ifndef NDDS_SAFETY_PROFILE
bool operator==(const NDDS::DCPS::Locator_t& x,
                const NDDS::DCPS::Locator_t& y)
{
  return x.kind == y.kind && x.port == y.port && std::memcmp(x.address, y.address, sizeof(x.address)) == 0;
}

bool operator==(const NDDS::DCPS::TransportLocator& x,
                const NDDS::DCPS::TransportLocator& y)
{
...
bool operator==(const NDDS::DCPS::LocatorSeq& x,
                const NDDS::DCPS::LocatorSeq& y)
{
  return sequence_equal(x, y);
}

bool operator==(const NDDS::DCPS::TransportLocatorSeq& x,
                const NDDS::DCPS::TransportLocatorSeq& y)
{
  return sequence_equal(x, y);
}

bool locatorsChanged(const NDDS::RTPS::ParticipantProxy_t& x,
                     const NDDS::RTPS::ParticipantProxy_t& y)
{
...
bool checkAndAssignLocators(NDDS::DCPS::WriterProxy_t& x,
                            const NDDS::DCPS::WriterProxy_t& y)
{
...
bool checkAndAssignLocators(NDDS::DCPS::ReaderProxy_t& x,
                            const NDDS::DCPS::ReaderProxy_t& y)
{
  if (!(x.allLocators == y.allLocators)) {
    x.allLocators = y.allLocators;
    return true;
...
#ifdef NDDS_SECURITY
bool is_stateless(const NDDS::DCPS::GUID_t& guid)
{
  return guid.entityId == NDDS::RTPS::ENTITYID_P2P_BUILTIN_PARTICIPANT_STATELESS_WRITER ||
    guid.entityId == NDDS::RTPS::ENTITYID_P2P_BUILTIN_PARTICIPANT_STATELESS_READER;
}

bool is_volatile(const NDDS::DCPS::GUID_t& guid)
{
  return guid.entityId == NDDS::RTPS::ENTITYID_P2P_BUILTIN_PARTICIPANT_VOLATILE_SECURE_WRITER ||
    guid.entityId == NDDS::RTPS::ENTITYID_P2P_BUILTIN_PARTICIPANT_VOLATILE_SECURE_READER;
}

bool is_stateless_or_volatile(const NDDS::DCPS::GUID_t& guid)
{
           ...
           XTypes::TypeLookupService_rch tls)
{
  ...
  const NDDS_STRING domainStr = DCPS::to_dds_string(domainId);
  const NDDS_STRING key = DCPS::GuidConverter(guid).uniqueParticipantId();

  // configure one transport
  transport_inst_ = TheTransportRegistry->create_inst(
                       DCPS::TransportRegistry::DEFAULT_INST_PREFIX +
                       NDDS_STRING("_SEDPTransportInst_") + key + domainStr,
                       "rtps_udp");
  ...
  // Create a config
  NDDS_STRING config_name = DCPS::TransportRegistry::DEFAULT_INST_PREFIX +
                            NDDS_STRING("_SEDP_TransportCfg_") + key +
                            domainStr;
...
Sedp::disassociate(DiscoveredParticipant& participant)
{
  ...
  NDDS_VECTOR(DiscoveredPublication) pubs_to_remove_from_bit;
  NDDS_VECTOR(DiscoveredSubscription) subs_to_remove_from_bit;
  ...
  for (NDDS_VECTOR(DiscoveredPublication)::iterator it = pubs_to_remove_from_bit.begin(); it != pubs_to_remove_from_bit.end(); ++it) {
    remove_from_bit_i(*it);
  }

  for (NDDS_VECTOR(DiscoveredSubscription)::iterator it = subs_to_remove_from_bit.begin(); it != subs_to_remove_from_bit.end(); ++it) {
    remove_from_bit_i(*it);
  }
...
void Sedp::remove_entities_belonging_to(
  Map& m, GUID_t participant, bool subscription, NDDS_VECTOR(typename Map::mapped_type)& to_remove_from_bit)
{
...
Sedp::update_topic_qos(const RepoId& topicId, const DDS::TopicQos& qos)
{
  ...
  NDDS_MAP_CMP(RepoId, NDDS_STRING, DCPS::GUID_tKeyLessThan)::iterator iter =
    topic_names_.find(topicId);
  if (iter == topic_names_.end()) {
    return false;
  }
  const NDDS_STRING& name = iter->second;
  ...
  if (qos.topic_data != topic.local_qos().topic_data) {
    ...
    for (RepoIdSet::const_iterator topic_endpoints = topic.local_publications().begin();
      ...
      NDDS_ASSERT(lp != local_publications_.end());
    ...
    for (RepoIdSet::const_iterator topic_endpoints = topic.local_subscriptions().begin();
      ...
      NDDS_ASSERT(ls != local_subscriptions_.end());
    ...
    if (iter == discovered_subscriptions_.end()) { // add new
      ...
      if (checkAndAssignLocators(iter->second.reader_data_.readerProxy, rdata.readerProxy)) {
        ...
        for (RepoIdSet::const_iterator i = assoc.begin(); i != assoc.end(); ++i) {
          ...
          NDDS_ASSERT(lpi != local_publications_.end());
          if (lpi != local_publications_.end()) {
            DCPS::DataWriterCallbacks_rch pl = lpi->second.publication_.lock();
            if (pl) {
  ...
  Serializer& ser, XTypes::TypeLookup_Reply& type_lookup_reply)
{
  ...
  if (NDDS_STRING(type_lookup_request.header.instanceName) != instance_name_) {
    return true;
  }
  ...
  const LocalPublication& pub)
{
  ...
  NDDS_STRING topic_name = topic_names_[pub.topic_id_];
  ...
  const LocalSubscription& sub)
{
  ...
  NDDS_STRING topic_name = topic_names_[sub.topic_id_];
  ...
  drd.contentFilterProperty.contentFilteredTopicName =
    NDDS_STRING(DCPS::GuidConverter(subscription_id)).c_str();
...
{
  const NDDS_STRING rtps_udp = "rtps_udp";
...
void Sedp::ignore(const GUID_t& to_ignore)
{
  ...
  {
    const NDDS_MAP_CMP(GUID_t, String, GUID_tKeyLessThan)::iterator
...
bool Sedp::remote_knows_about_local_i(const GUID_t& local, const GUID_t& remote) const
{
  ...
  if (gc.isBuiltinDomainEntity()) {
    NDDS_ASSERT(DCPS::GuidConverter(remote).isBuiltinDomainEntity());
...
void Sedp::match_continue(const GUID_t& writer, const GUID_t& reader)
{
...
#ifndef NDDS_NO_CONTENT_FILTERED_TOPIC
  ...
  if (writer_local) {
    ...
    NDDS_ASSERT(lpi->second.publication_);
    NDDS_ASSERT(dwr);
  }
  if (reader_local) {
    drr = lsi->second.subscription_;
    NDDS_ASSERT(lsi->second.subscription_);
    NDDS_ASSERT(drr);
  }
  ...
  if (compatibleQOS(&writerStatus, &readerStatus, *wTls, *rTls,
    ...
    if (call_reader && call_writer) {
      ...
      if (use_xtypes_complete_ && reader_type_info->complete.typeid_with_size.type_id.kind() == XTypes::TK_NONE) {
        ...
        NDDS::DCPS::RecorderImpl* ri = dynamic_cast<NDDS::DCPS::RecorderImpl*>(lock.in());
        if (ri) {
