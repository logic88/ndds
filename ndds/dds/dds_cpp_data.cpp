...
namespace {
  ...
  {
#ifdef NDDS_NO_OWNERSHIP_KIND_EXCLUSIVE
    ACE_UNUSED_ARG(qos);
    return false;
#else
  ...
  bool not_default(const NDDSParticipantFlags_t& flags)
  {
...
#ifdef NDDS_SECURITY
  NDDS::Security::DiscoveredParticipantDataKind find_data_kind(const ParameterList& param_list)
  {
    enum FieldMaskNames {
      ID_TOKEN_FIELD = 0x01,
    ...
    if ((field_mask & (ID_TOKEN_FIELD | PERM_TOKEN_FIELD)) == (ID_TOKEN_FIELD | PERM_TOKEN_FIELD)) {
      if ((field_mask & IDENTITY_STATUS_TOKEN_FIELD) == IDENTITY_STATUS_TOKEN_FIELD) {
        return NDDS::Security::DPDK_SECURE;
      } else {
        return NDDS::Security::DPDK_ENHANCED;
      }
    }

    return NDDS::Security::DPDK_ORIGINAL;
  }
#endif
  ...
  for (CORBA::ULong i = 0; i < length; ++i) {
    ...
    switch (param._d()) {
      ...
      case PID_PROPERTY_LIST:
        ...
        break;
      case PID_NDDS_PARTICIPANT_FLAGS:
        proxy.opendds_participant_flags = param.participant_flags();
        break;
      case PID_NDDS_RTPS_RELAY_APPLICATION_PARTICIPANT:
...
bool to_param_list(const NDDS::Security::SPDPdiscoveredParticipantData& participant_data,
                   ParameterList& param_list)
{
  if (participant_data.dataKind == NDDS::Security::DPDK_SECURE) {
    to_param_list(participant_data.ddsParticipantDataSecure, param_list);
  } else if (participant_data.dataKind == NDDS::Security::DPDK_ENHANCED) {
                   ...
                   bool map)
{
  ...
  // and multicastLocatorList.  This allows NDDS transports to be
  ...
  for (CORBA::ULong i = 0; i < locator_len; ++i) {
    ...
    if (!std::strcmp(tl.transport_type, "rtps_udp")) {
      ...
      // Otherwise, this is an NDDS, custom transport
    } else {
                   ...
                   bool map)
{
  ...
  for (i = 0; i < num_associations; ++i) {
    ...
    param._d(PID_NDDS_ASSOCIATED_WRITER);
    DCPS::push_back(param_list, param);
  }
  return true;
}
    ...
    switch (param._d()) {
      ...
      case PID_NDDS_LOCATOR:
        ...
        DCPS::push_back(reader_data.readerProxy.allLocators,
                       param.opendds_locator());
        break;
      case PID_NDDS_ASSOCIATED_WRITER:
                     ...
                     ICE::AgentInfoMap& ai_map)
{
  for (CORBA::ULong idx = 0, count = param_list.length(); idx != count; ++idx) {
    ...
    switch (parameter._d()) {
    case PID_NDDS_ICE_GENERAL: {
      const IceGeneral_t& ice_general = parameter.ice_general();
      ICE::AgentInfo& agent_info = ai_map[NDDS_STRING(ice_general.key.in())];
    ...
    case PID_NDDS_ICE_CANDIDATE: {
      ...
      ai_map[NDDS_STRING(ice_candidate.key.in())].candidates.push_back(candidate);
      break;
    }
    default:
      // Do nothing.
