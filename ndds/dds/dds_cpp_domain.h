...
#ifndef NDDS_DCPS_RTPS_RTPSDISCOVERY_H
#define NDDS_DCPS_RTPS_RTPSDISCOVERY_H
...
typedef NDDS_MAP_CMP(GUID_t, ParticipantHandle, GUID_tKeyLessThan) ParticipantMap;
typedef NDDS_MAP(DDS::DomainId_t, ParticipantMap) DomainParticipantMap;

const char RTPS_DISCOVERY_ENDPOINT_ANNOUNCEMENTS[] = "NDDS.RtpsDiscovery.EndpointAnnouncements";
const char RTPS_DISCOVERY_TYPE_LOOKUP_SERVICE[] = "NDDS.RtpsDiscovery.TypeLookupService";
const char RTPS_RELAY_APPLICATION_PARTICIPANT[] = "NDDS.Rtps.RelayApplicationParticipant";
const char RTPS_REFLECT_HEARTBEAT_COUNT[] = "NDDS.Rtps.ReflectHeartbeatCount";

class NDDS_Rtps_Export RtpsDiscoveryConfig : public NDDS::DCPS::RcObject {
public:
  typedef NDDS_VECTOR(NDDS_STRING) AddrVec;
  ...
  NDDS_STRING multicast_interface() const
  {
    ACE_GUARD_RETURN(ACE_Thread_Mutex, g, lock_, "");
    return multicast_interface_;
  }
  void multicast_interface(const NDDS_STRING& mi)
  {
    ACE_GUARD(ACE_Thread_Mutex, g, lock_);
  ...
  NDDS_STRING guid_interface() const
  {
    ACE_GUARD_RETURN(ACE_Thread_Mutex, g, lock_, "");
    return guid_interface_;
  }
  void guid_interface(const NDDS_STRING& gi)
  {
    ACE_GUARD(ACE_Thread_Mutex, g, lock_);
    guid_interface_ = gi;
  }
...
private:
  ...
  NDDS_STRING multicast_interface_;
...
typedef NDDS::DCPS::RcHandle<RtpsDiscoveryConfig> RtpsDiscoveryConfig_rch;
 ...
 *
 */
class NDDS_Rtps_Export RtpsDiscovery : public DCPS::Discovery {
public:
...
#    define NDDS_GCC_PRE53_DISABLE_OPTIMIZATION __attribute__((optimize("-O0")))
#  else
#    define NDDS_GCC_PRE53_DISABLE_OPTIMIZATION
#  endif
  ...
  NDDS_STRING multicast_interface() const { return config_->multicast_interface(); }
  void multicast_interface(const NDDS_STRING& mi) { config_->multicast_interface(mi); }
  ...
  NDDS_STRING guid_interface() const { return config_->guid_interface(); }
  void guid_interface(const NDDS_STRING& gi) { config_->guid_interface(gi); }
...
public:
  ...
  class NDDS_Rtps_Export StaticInitializer {
  public:
    StaticInitializer();
  };
...
typedef NDDS::DCPS::RcHandle<RtpsDiscovery> RtpsDiscovery_rch;
...
#endif /* NDDS_RTPS_RTPSDISCOVERY_H  */
